...
class SimpleLoggingCallback(Callback):
    ...
    def on_validation_end(self, recipe, **kwargs):
        ...
        for name, log_data in val_results.items():
            logger.info(f"[SimpleLoggingCallback] ✅ Validation '{name}': Loss = {log_data.metrics['val_loss']:.4f}")
    ...
    def on_train_end(self, recipe, **kwargs):
        logger.info(
            f"[SimpleLoggingCallback] 🎉 Training completed successfully! Final step: {recipe.step_scheduler.step}"
        )
...
class MetricsCollectorCallback(Callback):
    ...
    def on_validation_end(self, recipe, **kwargs):
        ...
        logger.info(f"[MetricsCollectorCallback] 📊 Collected {len(self.validation_metrics)} validation checkpoints")
