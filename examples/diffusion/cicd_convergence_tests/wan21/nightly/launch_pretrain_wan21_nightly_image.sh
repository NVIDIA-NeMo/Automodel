...
torchrun --nnodes=\$SLURM_NNODES --nproc-per-node=\$NUM_GPUS --rdzv_backend=c10d --rdzv_endpoint=\$MASTER_ADDR:\$MASTER_PORT examples/diffusion/pretrain/pretrain.py  -c examples/diffusion/pretrain/cicd/wan21_cicd_nightly_image.yaml
EOF
echo "$CMD"
