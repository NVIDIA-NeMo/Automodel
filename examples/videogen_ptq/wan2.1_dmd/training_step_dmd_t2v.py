# training_step_dmd_t2v.py - DMD Training Step with Alternating Optimization
...
def step_dmd_generator(
    ...
    if debug_mode or (global_step % 100 == 0):
        print0(f"[BACKWARD SIM] Starting from noise...")
    ...
    if debug_mode or (global_step % 100 == 0):
        print0(f"[BACKWARD SIM] Complete")
...
def step_dmd_critic(
    ...
    # Critic doesn't need negative embeddings (no CFG in critic loss)
    ...
    if debug_mode or (global_step % 100 == 0):
        print0(f"[BACKWARD SIM] Generated samples (detached)")
    ...
    # Check for NaN or explosion
    if torch.isnan(critic_loss) or critic_loss > 100:
