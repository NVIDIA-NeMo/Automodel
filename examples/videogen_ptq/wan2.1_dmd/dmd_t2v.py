...
class DMDT2V:
    ...
    def __init__(
        ...
        # Initialize inference pipeline (lazy initialization)
        self.inference_pipeline = None
        
        print0(f"[DMD] Initialized with:")
    ...
    def _compute_kl_grad(
        ...
        with torch.no_grad():
            ...
            pred_real_image = pred_real_cond + (
    ...
    def compute_distribution_matching_loss(
        ...
        with torch.no_grad():
            ...
            noisy_latent = self.scheduler.add_noise(
                generated_latent.flatten(0, 1).float(),
                noise.flatten(0, 1),
        ...
        if gradient_mask is not None:
            ...
            dmd_loss = 0.5 * F.mse_loss(
                ...
                (generated_latent.double() - grad.double()).detach(),
                reduction="mean"
            )
    ...
    def compute_critic_loss(
        ...
        if self.denoising_loss_type == "flow":
            ...
            critic_loss = F.mse_loss(
                pred_noise.float(),
                critic_noise.flatten(0, 1).float()
            )
    ...
    def _convert_x0_to_flow_pred(
        ...
        sigma = self.timestep_shift / (
            self.timestep_shift + (1.0 / t_norm - 1.0)
        )
