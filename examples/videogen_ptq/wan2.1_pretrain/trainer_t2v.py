...
class WanT2VTrainerFSDP:
    ...
    - Loss weighting with optional annealing
    - Karras-style sigma scheduling for pretraining
    ...
    def __init__(
        ...
        sigma_max: float = 1.0,
        rho: float = 7.0,
        loss_weight_k_start: float = 6.0,
        loss_weight_k_end: float = 2.0,
        anneal_loss_weight: bool = False,
        warmup_steps: int = 0,
        ...
        # Advanced pretrain config
        self.use_karras_schedule = use_karras_schedule
        self.sigma_min = sigma_min
        self.sigma_max = sigma_max
        self.rho = rho
        self.loss_weight_k_start = loss_weight_k_start
        self.loss_weight_k_end = loss_weight_k_end
        self.anneal_loss_weight = anneal_loss_weight
        self.warmup_steps = warmup_steps
        self.warmup_sigma_min_clamp = warmup_sigma_min_clamp
        ...
        mode = "PRETRAINING" if pretrain else "FINETUNING"
        print0(f"[INFO] WAN 2.1 T2V Trainer with Flow Matching - {mode} MODE")
        ...
        if use_sigma_noise:
            ...
            if use_karras_schedule:
                print0(f"[INFO]   - Sigma schedule: Karras (min={sigma_min}, max={sigma_max}, rho={rho})")
            else:
                print0(f"[INFO]   - Sigma schedule: Simple shift (shift={flow_shift})")
            print0(f"[INFO]   - Mix uniform ratio: {mix_uniform_ratio}")
            if anneal_loss_weight:
                print0(f"[INFO]   - Loss weight: Annealed (k: {loss_weight_k_start} -> {loss_weight_k_end})")
            else:
                print0(f"[INFO]   - Loss weight: Fixed (k={loss_weight_k_start})")
            if warmup_steps > 0:
                print0(f"[INFO]   - Warmup: {warmup_steps} steps (sigma >= {warmup_sigma_min_clamp})")
    ...
    def setup_pipeline(self):
        if self.pretrain:
            print0("[INFO] PRETRAINING MODE: Initializing transformer from config...")
            
            # Load transformer from config (random initialization)
            transformer = WanTransformer3DModel.from_config("./wan2.1_1.3B.config")
            
            # Load pipeline without VAE or text encoder, with random transformer
            ...
            print0("[INFO] Pipeline loaded with RANDOMLY INITIALIZED transformer")
            
        else:
            print0("[INFO] FINETUNING MODE: Loading pretrained transformer...")
            
            # Load pipeline with pretrained transformer
            self.pipe = WanPipeline.from_pretrained(
                self.model_id, 
                torch_dtype=torch.float32,
                vae=None,
            ...
            print0("[INFO] Pipeline loaded with PRETRAINED transformer")
        ...
        print0("[INFO] Pipeline setup complete (transformer + scheduler only)")
    ...
    def get_current_loss_weight_k(self, global_step: int, total_steps: int) -> float:
        """Calculate current loss weight k value with optional annealing."""
        if not self.anneal_loss_weight:
            return self.loss_weight_k_start
        
        # Cosine annealing
        progress = min(global_step / max(total_steps, 1), 1.0)
        k = (
            self.loss_weight_k_end + 
            0.5 * (self.loss_weight_k_start - self.loss_weight_k_end) * 
    ...
    def train(
        ...
        """
        mode = "PRETRAINING" if self.pretrain else "FINETUNING"
        print0(f"[INFO] Starting T2V {mode} with Flow Matching")
        ...
        if is_main_process():
            ...
            config = {
                ...
                # Advanced pretrain config
                "use_karras_schedule": self.use_karras_schedule,
                "sigma_min": self.sigma_min,
                "sigma_max": self.sigma_max,
                "rho": self.rho,
                "loss_weight_k_start": self.loss_weight_k_start,
                "loss_weight_k_end": self.loss_weight_k_end,
                "anneal_loss_weight": self.anneal_loss_weight,
                "warmup_steps": self.warmup_steps,
                "warmup_sigma_min_clamp": self.warmup_sigma_min_clamp,
            }
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for step, batch in enumerate(iterable):
                ...
                # Calculate current loss weight k (with annealing if enabled)
                current_k = self.get_current_loss_weight_k(global_step, total_steps)
