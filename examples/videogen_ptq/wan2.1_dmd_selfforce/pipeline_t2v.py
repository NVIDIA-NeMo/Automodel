...
class SelfForcingTrainingPipeline:
    ...
    def inference_with_trajectory(
        ...
        if exit_flags[0] == len(self.denoising_step_list) - 1:
            ...
            denoised_timestep_from = self._convert_to_scheduler_timestep(
                self.denoising_step_list[exit_flags[0]]
            )
        else:
            # Intermediate step
            denoised_timestep_to = self._convert_to_scheduler_timestep(
    ...
    def _convert_to_scheduler_timestep(self, discrete_step: int) -> int:
        ...
        timestep_idx = 1000 - torch.argmin(
            (self.scheduler.timesteps.to(discrete_step.device if isinstance(discrete_step, torch.Tensor) else 'cpu') - discrete_step).abs(),
            dim=0
        ).item()
        return timestep_idx
