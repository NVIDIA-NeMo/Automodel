...
class MetaFilesDataset(Dataset):
    ...
    def __init__(self, 
        ...
        # Find all .meta files
        self.meta_files = sorted(list(self.meta_folder.glob("*.meta")))[:8]
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        print0(f"[DEBUG] === DATA INSPECTION ===")
        print0(f"[DEBUG] Meta file: {meta_file.name}")
        print0(f"[DEBUG] Data keys: {list(data.keys())}")
        ...
        print0(f"[DEBUG] Original video_latents shape: {video_latents.shape}")
        print0(f"[DEBUG] Original video_latents dtype: {video_latents.dtype}")
        print0(f"[DEBUG] Video latents min/max: [{video_latents.min():.3f}, {video_latents.max():.3f}]")
        
        print0(f"[DEBUG] Original text_embeddings shape: {text_embeddings.shape}")
        print0(f"[DEBUG] Text embeddings dtype: {text_embeddings.dtype}")
        
        # Check metadata for preprocessing info
        if 'metadata' in data:
            metadata = data['metadata']
            print0(f"[DEBUG] Video metadata keys: {list(metadata.keys())}")
            if 'start_frame' in metadata and 'end_frame' in metadata:
                frame_count = metadata['end_frame'] - metadata['start_frame'] + 1
                print0(f"[DEBUG] Frame count from metadata: {frame_count}")
                print0(f"[DEBUG] Actual tensor frames: {video_latents.shape[-3] if video_latents.ndim >= 3 else 'UNKNOWN'}")
        
        # Check for preprocessing artifacts
        print0(f"[DEBUG] Deterministic latents: {data.get('deterministic_latents', 'UNKNOWN')}")
        print0(f"[DEBUG] Memory optimization: {data.get('memory_optimization', 'UNKNOWN')}")
        print0(f"[DEBUG] Original filename: {data.get('original_filename', 'UNKNOWN')}")
...
def create_dataloader(meta_folder: str,
                     batch_size: int = 4,
                     shuffle: bool = True,
                     num_workers: int = 0,
                     device: str = "cpu",
                     transform_text: Optional[callable] = None,
                     transform_video: Optional[callable] = None,
                     filter_fn: Optional[callable] = None) -> DataLoader:
    """
