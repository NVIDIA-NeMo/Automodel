...
class RoPEAwareContextParallelismManager:
    ...
    def _compute_rope_freqs(self, seq_len: int, device: torch.device, dtype: torch.dtype):
        ...
        freqs = 1.0 / (
            self.rope_base ** (torch.arange(0, self.rope_dim, 2, device=device, dtype=compute_dtype) / self.rope_dim)
        )
...
class RoPEAwareTransformerWrapper(torch.nn.Module):
    ...
    def forward(self, hidden_states, timestep, encoder_hidden_states, **kwargs):
        ...
        sharded_hidden, rope_metadata = self.cp_manager.shard_sequence_with_rope(hidden_states, dim=2)
        ...
        try:
            ...
            output = self.transformer(
                hidden_states=sharded_hidden, timestep=timestep, encoder_hidden_states=encoder_hidden_states, **kwargs
            )
            ...
            # Gather output
            gathered_output = self.cp_manager.gather_sequence_from_rope(main_output, rope_metadata, dim=2)
