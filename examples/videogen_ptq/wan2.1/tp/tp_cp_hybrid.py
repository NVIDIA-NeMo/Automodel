...
from lora_utils import (
    ...
    wan_install_lora_with_peft, collect_peft_lora_parameters,
)
...
def setup_tp_cp_for_pipe(pipe, device, bf16, local_rank: int, lora_rank: int, lora_alpha: int, 
                         use_peft: bool = False, train_transformer_2: bool = False, 
                         tp_size: int = None, cp_size: int = None):
    """
    ...
    for name in transformer_names:
        ...
        try:
            ...
            print0(f"[INFO] Successfully installed LoRA on {name}: {num_lora_modules} modules, {len(lora_params_before_parallel)} parameters")
        ...
        if cp_size > 1 and cp_manager is not None:
            ...
            print0(f"[INFO] RoPE-aware CP requires minimum {min_total_frames} total frames "
        ...
        if tp_size > 1 and device_mesh is not None:
            ...
            print0(f"[INFO] No TP applied (tp_size=1)")
        ...
        if cp_size > 1 and cp_manager is not None:
            ...
            print0(f"[INFO] No CP applied (cp_size=1)")
    ...
    for name in transformer_names:
        if name in model_map and model_map[name]["use_rope_cp"]:
            ...
            print0(f"  - For 100+ frames: RoPE will handle position encoding correctly across shards")
...
def apply_tp_to_transformer(transformer, device_mesh):
    ...
    print0(f"[TP] Applying TP to transformer (skipping FFN due to LoRA)...")
...
def test_tp_cp_forward_pass(model_map: Dict, transformer_names: List[str], device, bf16):
    ...
    for name in transformer_names:
        if name in model_map:
            ...
            dummy_encoder_hidden = torch.randn(batch_size, 77, 4096, 
                                              device=device, dtype=bf16)
            
            print0(f"[INFO] Testing {name} with {frames} frames (RoPE positioning across {model_map[name]['cp_size']} shards)")
    ...
    for name in transformer_names:
        if name in model_map and model_map[name]["use_rope_cp"]:
            ...
            if cp_manager:
                ...
                print0(f"  - Global sequence tracking: {'Enabled' if hasattr(cp_manager, 'global_seq_len') else 'Disabled'}")
            ...
            if isinstance(transformer, RoPEAwareTransformerWrapper):
                print0(f"  - RoPE-aware wrapper: Applied ✓")
            else:
                print0(f"  - RoPE-aware wrapper: Not applied ✗")
...
def setup_hybrid_for_pipe(pipe, device, bf16, local_rank: int, lora_rank: int, lora_alpha: int, 
    ...
    return setup_tp_cp_for_pipe(
        pipe, device, bf16, local_rank, lora_rank, lora_alpha, 
        use_peft, train_transformer_2, tp_size, cp_size
    )