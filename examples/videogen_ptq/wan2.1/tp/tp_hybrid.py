...
def setup_tp_for_pipe(pipe, device, bf16, local_rank: int, lora_rank: int, lora_alpha: int, use_peft: bool = False, train_transformer_2: bool = False):
    """
    ...
    for name in transformer_names:
        ...
        try:
            ...
            print0(f"[INFO] Successfully installed LoRA on {name}: {num_lora_modules} modules, {len(lora_params_before_tp)} parameters")
        ...
        print0(f"[INFO] LoRA parameters remain unsharded (replicated across all TP ranks)")
...
def test_tp_forward_pass(model_map: Dict, transformer_names: List[str], device, bf16):
    ...
    for name in transformer_names:
        ...
        try:
            ...
            print0(f"[INFO] {name} TP forward pass successful: input {hidden_states.shape} -> output {output_tensor.shape}")
...
def setup_hybrid_for_pipe(pipe, device, bf16, local_rank: int, lora_rank: int, lora_alpha: int, use_peft: bool = False, train_transformer_2: bool = False):
    """
