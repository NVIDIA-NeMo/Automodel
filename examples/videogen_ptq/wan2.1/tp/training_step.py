...
def step_single_transformer(
    ...
    print0("[DEBUG] Final conditioning shapes:")
    ...
    if active_transformer not in model_map:
        ...
        raise RuntimeError(
            f"Active transformer '{active_transformer}' not found in model_map. Available: {available_transformers}"
        )
    ...
    with torch.autocast(device_type="cuda", dtype=bf16):
        ...
        try:
            out = model(
                hidden_states=cond, timestep=timesteps, encoder_hidden_states=text_embeddings, return_dict=False
            )
            ...
            print0(
                f"[DEBUG] Input shapes - cond: {cond.shape}, timestep: {timesteps.shape}, text: {text_embeddings.shape}"
            )
            raise
        ...
        print0("[DEBUG]   Using first 16 channels for loss computation")
    ...
    if loss.item() == 0.0:
        print0("[WARNING] Zero loss detected - this might indicate a problem with conditioning masks")
