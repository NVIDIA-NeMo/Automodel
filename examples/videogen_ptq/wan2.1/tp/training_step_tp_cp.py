...
def step_dual_transformer(
    ...
    if video_latents.ndim == 5:
        ...
        raise ValueError(
            f"Unexpected video_latents shape: {video_latents.shape}. Expected 5D [B,C,T,H,W] or 6D [B,1,C,T,H,W]"
        )
    ...
    if active_transformer_name in model_map:
        ...
        if model_map[active_transformer_name].get("use_rope_cp", False):
            ...
            min_frames_needed = (
                model_map[active_transformer_name]["min_temporal_length"]
                * model_map[active_transformer_name]["cp_size"]
            )
    ...
    if active_transformer_name in model_map and model_map[active_transformer_name].get("use_rope_cp", False):
        min_frames_needed = (
            model_map[active_transformer_name]["min_temporal_length"] * model_map[active_transformer_name]["cp_size"]
        )
    ...
    conditioned_latents, noise, condition_mask = prepare_i2v_conditioning(pipe, video_latents, timesteps, bf16)

    # Verify conditioning output
    print0(f"[STEP] Conditioned latents shape: {conditioned_latents.shape}")
    print0("[STEP] Expected channels for transformer: checking...")
    ...
    try:
        ...
        if expected_channels and conditioned_latents.shape[1] != expected_channels:
            raise RuntimeError(
                f"Channel mismatch: conditioned={conditioned_latents.shape[1]}, expected={expected_channels}"
            )
    ...
    try:
        ...
        print0("[DEBUG] Expected input format: [B, C, T, H, W] = [batch, channels, frames, height, width]")
        raise
