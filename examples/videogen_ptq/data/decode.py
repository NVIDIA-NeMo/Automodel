...
class VideoDecoder:
    ...
    def decode_video_latents(self, video_latents: torch.Tensor) -> torch.Tensor:
        ...
        if hasattr(self.vae.config, 'latents_mean') and hasattr(self.vae.config, 'latents_std'):
            ...
            logger.warning(f"Using scaling_factor de-normalization: {scaling_factor}")
            logger.info(f"De-normalized latents range: [{video_latents.min():.3f}, {video_latents.max():.3f}]")
    ...
    def decode_meta_to_video(self, meta_path: str, output_path: str, fps: int = 24):
        """
        Complete pipeline: load .meta file and save as MP4.
    ...
    def decode_folder(self, meta_folder: str, output_folder: str, fps: int = 24):
        """
        Decode all .meta files in a folder to MP4 videos.
        
        Args:
            meta_folder: Path to folder containing .meta files
            output_folder: Path to folder for output MP4 files
        ...
        for i, meta_file in enumerate(meta_files):
            ...
            try:
                self.decode_meta_to_video(str(meta_file), str(output_file), fps)
            except Exception as e:
...
def main():
    ...
    parser.add_argument("--input", "-i", required=True, 
                        help="Input .meta file or folder containing .meta files")
    parser.add_argument("--output", "-o", required=True,
                        help="Output MP4 file or folder for MP4 files")
    ...
    parser.add_argument("--model", default="Wan-AI/Wan2.2-TI2V-5B-Diffusers",
                        help="Wan2.2 model ID (default: Wan-AI/Wan2.2-TI2V-5B-Diffusers)")
    ...
    if input_path.is_file() and input_path.suffix == '.meta':
        ...
        decoder.decode_meta_to_video(str(input_path), str(output_path), args.fps)
        
    elif input_path.is_dir():
        # Folder mode
        decoder.decode_folder(str(input_path), str(output_path), args.fps)
...
# Example usage:
"""
# Decode a single .meta file (with memory optimization)
...
# Entire folder
decoder.decode_folder("processed_meta", "decoded_videos", fps=24)
"""
