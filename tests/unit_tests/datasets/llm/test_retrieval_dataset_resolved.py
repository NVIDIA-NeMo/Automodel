...
class _FakeArrowTable:
    ...
    def fast_slice(self, offset, length):
        return _FakeArrowSlice(self.rows[offset : offset + length])


class _FakeHFDataset:
    def __init__(self, rows):
        self.data = _FakeArrowTable(rows)


class _FakeArrowCorpus:
    path = "fake_arrow_corpus"

    def __init__(self, rows):
        self.data = _FakeHFDataset(rows)
        self.docid2idx = {str(row["id"]): idx for idx, row in enumerate(rows)}

    def get_document_by_id(self, doc_id):
        raise AssertionError(f"decoded fallback should not be used for {doc_id}")

    def get_all_ids(self):
        return sorted(self.docid2idx)


class _FakeArrowCorpusInfo:
    def __init__(self, rows):
        self.metadata = {"corpus_id": "test", "query_instruction": "query:", "passage_instruction": "passage:"}
        self.corpus = _FakeArrowCorpus(rows)
...
def test_prepare_normalized_vl_retrieval_data_source_index_finalize(tmp_path, monkeypatch):
    pytest.importorskip("datasets")
    pytest.importorskip("datasets.arrow_writer")
    image_mod = pytest.importorskip("PIL.Image")
    _patch_normalized_source(monkeypatch, image_mod)
    ...
    data_dir_list = [{"path": "source_a.json", "num_samples": None}, {"path": "source_b.json", "num_samples": None}]
    source_1 = prep_norm.prepare_normalized_source(
        data_dir_list=data_dir_list,
        output_dir=output_dir,
        source_index=1,
        samples_per_shard=10,
    ...
    with pytest.raises(FileNotFoundError, match="source-00000"):
        prep_norm.finalize_normalized_sources(
            data_dir_list,
            output_dir,
            samples_per_shard=10,
            docs_per_shard=10,
    ...
    source_0 = prep_norm.prepare_normalized_source(
        data_dir_list=data_dir_list,
        output_dir=output_dir,
        source_index=0,
        samples_per_shard=10,
    ...
    metadata = prep_norm.finalize_normalized_sources(
        data_dir_list,
        output_dir,
        samples_per_shard=10,
        docs_per_shard=10,
...
def test_prepare_normalized_vl_retrieval_data_preserves_encoded_image_bytes(tmp_path, monkeypatch):
    pytest.importorskip("datasets")
    pytest.importorskip("datasets.arrow_writer")

    image_bytes = b"not-a-real-image-but-already-encoded"
    monkeypatch.setattr(
        prep_norm,
        "load_datasets",
        lambda data_dir_list, concatenate, seed: (
            ...
            ],
            {
                "test": _FakeCorpusInfo(
                    {
                        "p": {"text": "positive", "image": {"bytes": image_bytes, "path": None}},
                        "n": {"text": "negative", "image": ""},
                    }
                )
            },
    ...
    output_dir = tmp_path / "normalized_encoded"
    ...
    source_dir = output_dir / "sources" / "source-00000"
    shard = datasets.Dataset.from_file(str(next((source_dir / "corpus" / "test").glob("*.arrow"))))
    packed_images = {row["id"]: bytes(row["image_jpeg"]) for row in shard}
    assert packed_images["p"] == image_bytes
    assert packed_images["n"] == b""


def test_prepare_normalized_vl_retrieval_data_reads_arrow_image_bytes_without_decoding(tmp_path, monkeypatch):
    pytest.importorskip("datasets")
    pytest.importorskip("datasets.arrow_writer")

    image_bytes = b"encoded-arrow-image"
    monkeypatch.setattr(
        prep_norm,
        "load_datasets",
        lambda data_dir_list, concatenate, seed: (
            ...
            {
                "test": _FakeArrowCorpusInfo(
                    [
                        {"id": "p", "text": "positive", "image": {"bytes": image_bytes, "path": None}},
                        {"id": "n", "text": "negative", "image": ""},
                    ]
                )
            },
        ),
    ...
    output_dir = tmp_path / "normalized_arrow_raw"
    prep_norm.prepare_normalized_dataset(
        data_dir_list=["train.json"],
        output_dir=output_dir,
        samples_per_shard=10,
    ...
    source_dir = output_dir / "sources" / "source-00000"
    shard = datasets.Dataset.from_file(str(next((source_dir / "corpus" / "test").glob("*.arrow"))))
    rows = {row["id"]: row for row in shard}
    assert bytes(rows["p"]["image_jpeg"]) == image_bytes
    assert rows["p"]["text"] == "positive"
    assert rows["n"]["image_jpeg"] == b""


def test_prepare_normalized_vl_retrieval_data_reads_docmatix_arrow_image_bytes():
    image_bytes = b"docmatix-encoded-image"
    corpus = type("DocMatixDataset", (), {})()
    corpus.data = _FakeHFDataset(
        [{"images": [{"bytes": b"first", "path": None}, {"bytes": image_bytes, "path": None}]}]
    )
    corpus_info = types.SimpleNamespace(corpus=corpus)

    doc = prep_norm._document_from_arrow_storage(corpus_info, "0_1")

    assert doc == {"image": {"bytes": image_bytes, "path": None}}
...
def test_prepare_normalized_vl_retrieval_data_resume_rejects_changed_source(tmp_path, monkeypatch):
    pytest.importorskip("datasets")
    pytest.importorskip("datasets.arrow_writer")
    image_mod = pytest.importorskip("PIL.Image")
    _patch_normalized_source(monkeypatch, image_mod)
    ...
    with pytest.raises(ValueError, match="different source entry"):
        prep_norm.prepare_normalized_dataset(
            data_dir_list=[{"path": "source_b.json", "num_samples": None}],
            output_dir=output_dir,
            samples_per_shard=10,
