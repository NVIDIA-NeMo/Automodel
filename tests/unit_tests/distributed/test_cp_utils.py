...
def test_make_cp_batch_and_ctx_seq_index_added_to_buffers(monkeypatch):
    """When ``seq_index`` is present it is appended as a CP buffer sharded on dim 1."""
    ...
    device_mesh = _DummyDeviceMesh(cp_size=2, tp_size=1)
    seq_len = 8  # divisible by cp_size*2=4 so no padding is triggered
    ...
    # input_ids dim=1, labels dim=1, position_ids dim=1, seq_index dim=1
    assert captured_kwargs["cp_seq_dims"] == [1, 1, 1, 1]
    assert any(t is seq_index for t in captured_kwargs["cp_buffers"]), "seq_index must be in cp_buffers"


def test_make_cp_batch_and_ctx_no_seq_index_keeps_buffers_unchanged(monkeypatch):
    """Without ``seq_index`` the buffer set must not gain an extra entry (opt-in)."""
    ...
    # Only input_ids, labels, position_ids — no seq_index buffer.
    assert captured_kwargs["cp_seq_dims"] == [1, 1, 1]


def test_make_cp_batch_and_ctx_singleton_seq_index_expands_to_batch(monkeypatch):
    """A ``[1, S]`` seq_index expands to ``[B, S]`` when batch_size > 1."""
    ...
    seq_len = 8
    batch = {
        "input_ids": torch.arange(2 * seq_len).view(2, seq_len),
        "labels": torch.arange(2 * seq_len).view(2, seq_len),
        "seq_index": torch.arange(seq_len).unsqueeze(0),  # [1, S]
    }

    _cu.make_cp_batch_and_ctx(device_mesh, batch)

    assert batch["seq_index"].shape == (2, seq_len), "singleton seq_index should expand to batch size"
    # Both rows hold the same arange.
    assert torch.equal(batch["seq_index"][0], batch["seq_index"][1])


def test_make_cp_batch_and_ctx_seq_index_padding_continues_arange(monkeypatch):
    """seq_index padding must continue the arange (seq_len .. seq_len+pad_len), not pad with 0."""
    ...
    padded_seq_index = batch["seq_index"]
    assert padded_seq_index.shape[1] == 8
    # Original positions preserved, pad region continues the index.
    assert padded_seq_index[0].tolist() == [0, 1, 2, 3, 4, 5, 6, 7]
