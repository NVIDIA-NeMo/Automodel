...
def test_process_input_for_thd_preserves_unrecognized_fields():
    ...
    batch = {
        ...
        "labels": torch.arange(6).view(2, 3),
...
def test_split_batch_into_thd_chunks_splits_arbitrary_batch_tensors():
    ...
    batch = {
        "input_ids": torch.arange(1, 9).view(2, 4),
        "labels": torch.arange(1, 9).view(2, 4),
...
def test_split_single_packed_row_rejects_variable_pipeline_token_widths():
    batch = {
        "input_ids": torch.arange(8).unsqueeze(0),
        "labels": torch.arange(8).unsqueeze(0),
        "position_ids": torch.arange(8).unsqueeze(0),
        "seq_lens": torch.tensor([[1, 1, 2, 4]]),
        "seq_lens_padded": torch.tensor([[1, 1, 2, 4]]),
    }

    with pytest.raises(ValueError, match="equal token widths"):
        split_batch_into_thd_chunks(batch, num_chunks=2)


def test_stack_thd_chunks_rejects_inconsistent_fields():
    from nemo_automodel.components.distributed.thd_utils import stack_thd_chunks

    chunks = [
        {"input_ids": torch.tensor([1]), "cu_seqlens": torch.tensor([0, 1])},
        {
            "input_ids": torch.tensor([2]),
            "cu_seqlens": torch.tensor([0, 1]),
            "unexpected": torch.tensor([3]),
        },
    ]

    with pytest.raises(ValueError, match="inconsistent field 'unexpected'"):
        stack_thd_chunks(chunks)
