...
class TestValidation:
    ...
    def test_megatron_fsdp_rejects_pp(self):
        with pytest.raises(ValueError, match="pipeline parallelism"):
            parse_distributed_section({"strategy": "megatron_fsdp", "pp_size": 2})

    def test_megatron_fsdp_rejects_ep(self):
        with pytest.raises(ValueError, match="expert parallelism"):
            parse_distributed_section({"strategy": "megatron_fsdp", "ep_size": 2})

    def test_megatron_fsdp_rejects_sequence_parallel(self):
        with pytest.raises(ValueError, match="sequence_parallel"):
            parse_distributed_section({"strategy": "megatron_fsdp", "sequence_parallel": True})

    def test_ddp_rejects_tp(self):
        with pytest.raises(ValueError, match="tensor parallelism"):
            parse_distributed_section({"strategy": "ddp", "tp_size": 2})

    def test_ddp_rejects_pp(self):
        with pytest.raises(ValueError, match="pipeline parallelism"):
            parse_distributed_section({"strategy": "ddp", "pp_size": 2})

    def test_ddp_rejects_cp(self):
        with pytest.raises(ValueError, match="context parallelism"):
            parse_distributed_section({"strategy": "ddp", "cp_size": 2})

    def test_ddp_rejects_ep(self):
        with pytest.raises(ValueError, match="expert parallelism"):
            parse_distributed_section({"strategy": "ddp", "ep_size": 2})

    def test_ddp_rejects_hsdp(self):
        with pytest.raises(ValueError, match="HSDP"):
            parse_distributed_section({"strategy": "ddp", "dp_replicate_size": 2})
...
class TestIntegration:
    def test_strategy_map_entries(self):
        assert STRATEGY_MAP == {"fsdp2": FSDP2Config, "megatron_fsdp": MegatronFSDPConfig, "ddp": DDPConfig}
