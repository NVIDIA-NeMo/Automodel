...
class TestPreFSDPHook:
    def test_hook_resets_meta_head_after_post_shard_base_load(self):
        ...
        def materialize_model(model_to_materialize, device, **_kwargs):
            ...
            # Architecture initializers may cast the whole sharded model to the
            # backbone dtype. Infrastructure must restore the isolated task head
            # to FP32 before sampling its fresh initialization.
            model_to_materialize.to(dtype=torch.bfloat16)
        ...
        def add_value_head(model_to_update):
            assert not hasattr(model_to_update, "base_checkpoint_loaded")
            assert all(parameter.device.type == "meta" for parameter in model_to_update.parameters())

            value_head = torch.nn.Linear(4, 1)
            assert value_head.weight.device.type == "meta"
            original_reset = value_head.reset_parameters

            def reset_after_base_load():
                assert model_to_update.base_checkpoint_loaded is True
                assert all(parameter.device.type == "cpu" for parameter in value_head.parameters())
                assert all(parameter.dtype == torch.float32 for parameter in value_head.parameters())
                original_reset()
    ...
    @pytest.mark.parametrize(
        "invalid_declaration,error_match",
        [
            ("reused", "reuses pre-hook parameters"),
            ("existing_module", "existed before the hook"),
            ("unattached", "attached below the model root"),
            ("external_alias", "shares parameters outside its subtree"),
            ("bf16", "only float32 parameters"),
            ("frozen", "only trainable parameters"),
        ],
    ...
    def test_returned_module_validates_fresh_attached_module(
        ...
        if invalid_declaration == "existing_module":
            model.value_head = torch.nn.Identity()
        ...
        with (
            ...
            patch(f"{_INFRA_MODULE}.Checkpointer"),
        ):
    ...
    def test_rejects_invalid_hook_result_before_key_snapshot_or_sharding(self):
        from nemo_automodel._transformers import infrastructure as infra
    ...
    @pytest.mark.parametrize(
        "infrastructure_overrides,native_quantization",
        [
            ({"peft_config": object()}, False),
            ({"quantization_config": object()}, False),
            ({"fp8_config": object()}, False),
            ({"qat_quantizer": object()}, False),
            ({}, True),
        ],
        ids=["peft", "quantization", "fp8", "qat", "native_quantization"],
    )
    def test_managed_result_runs_after_and_is_allowed_with_model_transforms(
        ...
        def apply_transforms(model_to_update, *_args):
            model_to_update.transforms_complete = True
        ...
        with (
            patch(f"{_INFRA_MODULE}.get_world_size_safe", return_value=1),
            patch(f"{_INFRA_MODULE}._apply_peft_and_lower_precision", transform),
            patch(f"{_INFRA_MODULE}._supports_logits_to_keep", return_value=True),
            patch(f"{_INFRA_MODULE}.print_trainable_parameters"),
            patch(f"{_INFRA_MODULE}._should_load_before_shard", return_value=False),
        ...
        assert result is model
        assert [mock_call[0] for mock_call in timeline.mock_calls] == ["transform", "hook"]
        assert task_head_module_name(model) == "value_head"
        assert model.value_head.weight.dtype == torch.float32
        assert all(parameter.requires_grad for parameter in model.value_head.parameters())

    def test_pipeline_parallelism_rejects_hook_before_it_runs(self):
        from nemo_automodel._transformers import infrastructure as infra
        ...
        with pytest.raises(NotImplementedError, match="pipeline parallelism"):
            infra.apply_model_infrastructure(
                ...
                device=torch.device("cpu"),
                load_base_model=False,
                mesh=SimpleNamespace(pp_size=2),
                pre_fsdp_hook=hook,
            )
    ...
    def test_managed_result_rejects_tied_output_head(self):
        ...
        with (
            ...
            with pytest.raises(ValueError, match="untied output head"):
                infra.apply_model_infrastructure(
                    model=model,
                    is_meta_device=False,
                    device=torch.device("cpu"),
    ...
    def test_peft_freeze_keeps_lora_and_managed_task_head_trainable(self):
        ...
        def apply_peft(model_to_update, *_args):
            model_to_update.linear.register_parameter("lora_A", torch.nn.Parameter(torch.ones(1)))
            return model_to_update
        ...
        with (
            patch(f"{_INFRA_MODULE}.get_world_size_safe", return_value=1),
            patch(f"{_INFRA_MODULE}._apply_peft_and_lower_precision", side_effect=apply_peft),
            patch(f"{_INFRA_MODULE}._supports_logits_to_keep", return_value=True),
            patch(f"{_INFRA_MODULE}.print_trainable_parameters"),
            patch(f"{_INFRA_MODULE}._should_load_before_shard", return_value=False),
            patch(f"{_INFRA_MODULE}._shard_ep_fsdp", return_value=model),
...
def test_shard_ep_fsdp_wraps_fp32_task_head_on_size_one_data_mesh_for_pure_tp():
    from nemo_automodel._transformers import infrastructure as infra
    from nemo_automodel.components.distributed.fsdp2 import FSDP2Manager

    model = _DummyModel()
    task_head = _register_test_task_head(model)
    manager = object.__new__(FSDP2Manager)
    manager.offload_policy = object()
    data_mesh = SimpleNamespace(size=lambda: 1)
    world_mesh = object()
    parallelize_fn = MagicMock()
    mesh = SimpleNamespace(
        device_mesh=world_mesh,
        moe_mesh=None,
        parallelize_axis_kwargs=MagicMock(return_value={}),
    )

    with (
        patch(f"{_INFRA_MODULE}.get_world_size_safe", return_value=4),
        patch(f"{_INFRA_MODULE}.get_fsdp_dp_mesh", return_value=data_mesh) as get_data_mesh,
        patch(f"{_INFRA_MODULE}.MixedPrecisionPolicy", return_value="task-head-fp32") as policy,
        patch(f"{_INFRA_MODULE}.fully_shard") as fully_shard,
    ):
        result = infra._shard_ep_fsdp(model, manager, parallelize_fn, mesh)

    assert result is model
    assert data_mesh.size() == 1
    assert all(parameter.dtype == torch.float32 for parameter in task_head.parameters())
    get_data_mesh.assert_called_once_with(world_mesh, "dp_replicate", "dp_shard_cp")
    ...
    fully_shard.assert_called_once_with(
        task_head,
        mesh=data_mesh,
        mp_policy="task-head-fp32",
        offload_policy=manager.offload_policy,
        reshard_after_forward=False,
    )
    parallelize_fn.assert_called_once_with(model, world_mesh=world_mesh, moe_mesh=None)


def test_sync_task_head_replicas_broadcasts_over_tp_but_not_ep():
    from nemo_automodel._transformers import infrastructure as infra
    ...
    class _RootMesh:
        ...
        def __getitem__(self, axis):
            ...
            return {"tp": _Submesh(tp_group), "ep": _Submesh(ep_group)}[axis]

    root_mesh = _RootMesh()
    mesh = SimpleNamespace(device_mesh=root_mesh)

    with (
        patch(f"{_INFRA_MODULE}.dist.is_available", return_value=True),
        patch(f"{_INFRA_MODULE}.dist.is_initialized", return_value=True),
        patch(f"{_INFRA_MODULE}.dist.get_process_group_ranks", return_value=[7, 9]),
        patch(f"{_INFRA_MODULE}.dist.get_backend", return_value="gloo"),
        patch(f"{_INFRA_MODULE}.dist.broadcast") as broadcast,
    ):
        infra._sync_task_head_replicas(model, mesh, torch.device("cpu"))

    assert root_mesh.requested_axes == ["tp"]
    assert broadcast.call_count == len(tuple(task_head.parameters()))
    assert all(mock_call.kwargs == {"src": 7, "group": tp_group} for mock_call in broadcast.call_args_list)


def _run_two_rank_task_head_tp_sync(rank, init_file):
    import torch.distributed as dist
    from torch.distributed.device_mesh import init_device_mesh

    from nemo_automodel._transformers.infrastructure import _sync_task_head_replicas
    ...
    try:
        ...
        with torch.no_grad():
            for parameter in task_head.parameters():
                parameter.fill_(10.0 + rank)

        world_mesh = init_device_mesh("cpu", mesh_shape=(1, 2), mesh_dim_names=("ep", "tp"))
        _sync_task_head_replicas(
            model,
            SimpleNamespace(device_mesh=world_mesh),
            torch.device("cpu"),
        )

        for parameter in task_head.parameters():
            torch.testing.assert_close(parameter, torch.full_like(parameter, 10.0), rtol=0.0, atol=0.0)
    finally:
        dist.destroy_process_group()


def test_sync_task_head_replicas_two_rank_gloo(tmp_path):
    torch.multiprocessing.spawn(
        _run_two_rank_task_head_tp_sync,
        args=(str(tmp_path / "task_head_tp_sync"),),
        nprocs=2,
        join=True,
    )
