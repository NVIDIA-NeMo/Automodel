...
def test_patch_protocols_are_structural():
    """Patch modules opt into lifecycle hooks by defining convention functions."""
    from nemo_automodel.components.models.protocols import (
        GlobalPatchModule,
        PostShardPatchModule,
        PostShardPatchPredicate,
        PreConfigPatchModule,
        RuntimePatchSpecModule,
    )

    module = types.SimpleNamespace(
        apply_global_patches=lambda: None,
        apply_pre_config_patches=lambda: None,
        get_runtime_patch_specs=lambda: {"FakeArch": ("fake.module", "patch")},
        apply_post_shard_patches=lambda model_parts: None,
        should_apply_post_shard_patches=lambda model_parts: bool(model_parts),
    )

    assert isinstance(module, GlobalPatchModule)
    assert isinstance(module, PreConfigPatchModule)
    assert isinstance(module, RuntimePatchSpecModule)
    assert isinstance(module, PostShardPatchModule)
    assert isinstance(module, PostShardPatchPredicate)
