...
"""Tests that Qwen3MoeForCausalLM supports memory-efficient fused cross-entropy.
...
@pytest.fixture
def model() -> Qwen3MoeForCausalLM:
    m = Qwen3MoeForCausalLM(_tiny_config(), backend=_cpu_backend())
    # Initialize weights so forward produces finite values (random nn.Module init can
    # yield NaNs through the MoE softmax gate on this tiny config).
