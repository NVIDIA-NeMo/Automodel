...
"""Unit tests for Qwen4-Exp checkpoint configuration and registry wiring."""
...
from nemo_automodel.components.models.qwen4_exp.config import (
    Qwen4ExpConfig,
    Qwen4ExpTextConfig,
...
def test_checkpoint_style_auto_config_resolves_local_nested_configs(tmp_path) -> None:
    ...
    payload["auto_map"] = {"AutoConfig": "configuration_qwen4_exp.Qwen4ExpConfig"}
    ...
    assert type(config) is Qwen4ExpConfig
    assert type(config.text_config) is Qwen4ExpTextConfig
    assert type(config.vision_config) is Qwen4ExpVisionConfig
...
def test_custom_config_registry_covers_outer_and_text_model_types() -> None:
    assert resolve_custom_config_cls("qwen4_exp") is Qwen4ExpConfig
    assert resolve_custom_config_cls("qwen4_exp_text") is Qwen4ExpTextConfig
    assert _CUSTOM_CONFIG_REGISTRATIONS["qwen4_exp"] == (
        "nemo_automodel.components.models.qwen4_exp.config",
        "Qwen4ExpConfig",
    )
    assert _CUSTOM_CONFIG_REGISTRATIONS["qwen4_exp_text"] == (
        "nemo_automodel.components.models.qwen4_exp.config",
        "Qwen4ExpTextConfig",
    )


def test_architecture_registry_points_to_qwen4_exp_model() -> None:
    assert MODEL_ARCH_MAPPING["Qwen4ExpForConditionalGeneration"] == (
        "nemo_automodel.components.models.qwen4_exp.model",
...
def test_sglang_compatible_defaults_do_not_enable_ple() -> None:
    config = Qwen4ExpTextConfig(num_hidden_layers=4)
...
def test_nested_config_instances_and_serialization_are_preserved() -> None:
    text_config = Qwen4ExpTextConfig(num_hidden_layers=4, ple_layer_ids=[2])
    vision_config = Qwen4ExpVisionConfig(depth=2)
    config = Qwen4ExpConfig(text_config=text_config, vision_config=vision_config)

    assert config.text_config is text_config
    assert config.vision_config is vision_config

    payload = config.to_dict()
    assert payload["model_type"] == "qwen4_exp"
    assert payload["text_config"]["model_type"] == "qwen4_exp_text"
    assert payload["text_config"]["num_hidden_layers"] == 4
    assert payload["vision_config"]["model_type"] == "qwen4_exp"
