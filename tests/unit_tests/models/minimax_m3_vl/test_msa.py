...
def test_optional_dependencies_are_lazy() -> None:
    ...
    # Do not reject cutlass here: TE 2.19 imports its FA4 backend when FA4 is installed, and that
    # backend legitimately imports cutlass without loading any of the model-private MSA kernels.
...
class RejectGpuImports:
    def find_spec(self, fullname, path=None, target=None):
        if fullname.split(".")[0] in {"fmha_sm100", "quack"}:
