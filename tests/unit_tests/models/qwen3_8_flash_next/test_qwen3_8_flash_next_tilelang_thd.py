...
"""CPU contract and H100 parity tests for direct-THD Qwen3.8-Flash-Next sparse GQA."""
...
def test_thd_kernel_modules_import_without_tilelang(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    module_names = (
        "nemo_automodel.components.models.qwen3_8_flash_next.kernels.sparse_attention_thd",
        "nemo_automodel.components.models.qwen3_8_flash_next.kernels.tilelang_sparse_gqa_thd_bwd",
        "nemo_automodel.components.models.qwen3_8_flash_next.kernels.tilelang_sparse_gqa_thd_fwd",
    )
    ...
    with pytest.raises(UnavailableError, match="Qwen3.8-Flash-Next TileLang kernels"):
...
def test_thd_public_api_normalizes_route_layout_without_narrowing(
    ...
    module = importlib.import_module("nemo_automodel.components.models.qwen3_8_flash_next.kernels.sparse_attention_thd")
    ...
    monkeypatch.setattr(module._Qwen3_8_FlashNextSparseGQAThdAttention, "apply", fake_apply)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("kv_heads", ValueError, r"key/value must be \[Tkv, 2, D\]"),
...
def _bshd_attention(
    ...
    Returns:
        BF16 attention output of shape ``[tokens, 24, 256]``.
    """
    from nemo_automodel.components.models.qwen3_8_flash_next.kernels.sparse_attention import (
...
@requires_h100_tilelang
def test_thd_tilelang_cp_sharded_queries_match_full_run() -> None:
    """Prove decoupled Tq/Tkv reproduces contiguous context-parallel sharding.

    A rank in packed-sequence CP holds a contiguous query shard while K/V and
    the indexer routes stay in the gathered global token space. Running the
    kernel per shard must reproduce the full run: output and dQ equal the
    full-run slices, and the FP32-accumulated shard dK/dV sum to the full-run
    KV gradients.
    """
    cu_seqlens = (0, 5, 12, 32)
    tensors = _make_cuda_inputs(cu_seqlens, topk=9, seed=505)
    grad_output = _noncontiguous_grad_output(tensors[0], seed=506)
    full = _forward_backward(_direct_attention, tensors, grad_output)

    query, key, value, token_ids = tensors
    tokens = query.shape[0]
    shard_boundary = tokens // 2
    assert shard_boundary not in cu_seqlens, "the shard split must cut through a document"

    output_shards: list[torch.Tensor] = []
    dquery_shards: list[torch.Tensor] = []
    dkey_sum = torch.zeros_like(full[2], dtype=torch.float32)
    dvalue_sum = torch.zeros_like(full[3], dtype=torch.float32)
    for shard_start, shard_end in ((0, shard_boundary), (shard_boundary, tokens)):
        shard_tensors = (
            query[shard_start:shard_end].contiguous(),
            key,
            value,
            token_ids[shard_start:shard_end].contiguous(),
        )
        shard_grad = grad_output[shard_start:shard_end].contiguous()
        shard_output, shard_dquery, shard_dkey, shard_dvalue = _forward_backward(
            _direct_attention, shard_tensors, shard_grad
        )
        assert shard_output.shape == (shard_end - shard_start, _QUERY_HEADS, _HEAD_DIM)
        assert shard_dkey.shape == key.shape
        output_shards.append(shard_output)
        dquery_shards.append(shard_dquery)
        dkey_sum += shard_dkey.float()
        dvalue_sum += shard_dvalue.float()

    actual = (
        torch.cat(output_shards, dim=0),
        torch.cat(dquery_shards, dim=0),
        dkey_sum.to(full[2].dtype),
        dvalue_sum.to(full[3].dtype),
    )
    _assert_parity(actual, full)
