...
class TestEncoderStateDictAdapter:
    """Test suite for EncoderStateDictAdapter."""

    @pytest.fixture
    def adapter(self):
        """Create an EncoderStateDictAdapter instance for testing."""
    ...
    def test_from_hf_empty_state_dict(self, adapter):
        """Test conversion with empty state dict."""
        encoder_state_dict = adapter.from_hf({})
        assert encoder_state_dict == {}
    ...
    def test_roundtrip_conversion(self, adapter):
        ...
        # HF -> encoder
        encoder_state = adapter.from_hf(original_hf_state)

        # encoder -> HF (should recover original)
        recovered_hf_state = adapter.to_hf(encoder_state)
