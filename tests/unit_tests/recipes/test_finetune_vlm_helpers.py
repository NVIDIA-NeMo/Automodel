...
@pytest.mark.parametrize("use_te", [None, False, True])
def test_run_train_step_supports_tensor_outputs(monkeypatch: pytest.MonkeyPatch, use_te: bool | None) -> None:
    """Check tensor outputs and explicit TE selection through a native optimizer step."""
    ...
    recipe.cfg = ConfigNode({"fp8": None, **({"clip_grad_norm": {"use_te": use_te}} if use_te is not None else {})})
    ...
    assert grad_clip_mock.call_args.kwargs["use_te"] is (use_te is True)
