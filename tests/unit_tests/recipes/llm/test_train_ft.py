...
def test_validation_dataloaders_collects_and_names_properly():
    ...
    ds_target = "tests.unit_tests.recipes.llm.test_train_ft.DummyIterableDataset"
    cfg = ConfigNode(
        {
            "model": {},
            "validation_dataloader": {},
    ...
    cfg = ConfigNode(
        {
            ...
            "validation_dataset": {"_target_": "tests.unit_tests.recipes.llm.test_train_ft.DummyMapDataset"},
        }
    )
...
def test_should_pack_validation_for_explicit_thd_collater(attn):
    ...
    dataset = {"_target_": "tests.unit_tests.recipes.llm.test_train_ft.DummyMapDataset"}
    cfg = RecipeConfig(
        ConfigNode(
            {
                "model": {"backend": {"attn": attn}},
...
def test_build_loader_iterable_shard_and_shuffle_removed_from_cfg(monkeypatch):
    ...
    cfg_ds = ConfigNode(
        {
            "_target_": "tests.unit_tests.recipes.llm.test_train_ft.DummyIterableDataset",
            "tokenizer": None,
            "num_shards": 4,
        }
    )
...
def test_build_dataloader_packing_uses_configured_cp_size(monkeypatch, supports_thd):
    ...
    cfg_ds = ConfigNode(
        {
            "_target_": "tests.unit_tests.recipes.llm.test_train_ft.DummyMapDataset",
            "split": "train",
        }
    )
    cfg_dl = ConfigNode(
...
def test_build_dataloader_parallel_tokenize_gated_on_num_proc(monkeypatch, num_proc, max_packs, expect_parallel):
    ...
    cfg_ds = ConfigNode({"_target_": "tests.unit_tests.recipes.llm.test_train_ft.DummyMapDataset", "split": "train"})
