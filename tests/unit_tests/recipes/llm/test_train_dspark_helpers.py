...
def test_metric_window_omits_only_the_unmeasured_positions():
    # A block whose later positions never carried a valid token still logs the
    # positions that did (the per-position gate is on each position's own denominator).
    window = _DSparkMetricWindow(block_size=3)
