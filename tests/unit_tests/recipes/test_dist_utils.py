...
from nemo_automodel.recipes._dist_utils import create_mesh_context_from_config, parse_distributed_section
...
class TestParsing:
    ...
    def test_megatron_fsdp_alias(self):
        result = parse_distributed_section({"strategy": "mfsdp"})
        assert isinstance(result["strategy_config"], MegatronFSDPConfig)
...
# ---------------------------------------------------------------------------
# create_mesh_context_from_config: world_size auto-detection
# ---------------------------------------------------------------------------


class TestCreateMeshContextFromConfigWorldSizeAutoDetect:
    """``create_mesh_context_from_config`` accepts an optional ``world_size`` and auto-detects
    it from ``torch.distributed`` / ``WORLD_SIZE`` when not provided."""

    @pytest.fixture
    def patched_mesh(self, monkeypatch):
        """Stub create_mesh_context to capture the world_size it receives."""
        captured: dict = {}

        def fake_create_mesh_context(strategy_config, **kwargs):
            captured.update(kwargs)
            return {"strategy_config": strategy_config, **kwargs}

        monkeypatch.setattr(
            "nemo_automodel.components.distributed.device_mesh.create_mesh_context",
            fake_create_mesh_context,
        )
        return captured

    def test_explicit_world_size_used(self, patched_mesh):
        create_mesh_context_from_config({"strategy": "fsdp2"}, world_size=4)
    ...
    def test_auto_detect_from_env(self, monkeypatch, patched_mesh):
        ...
        create_mesh_context_from_config({"strategy": "fsdp2"})
    ...
    def test_programmatic_args_without_cfg(self, patched_mesh):
        result = create_mesh_context_from_config(
            strategy="fsdp2",
            tp_size=2,
            ep_size=2,
            activation_checkpointing=True,
