...
# Enable wandb in CI: the recipes ship `wandb.enable: false` (example-yaml linter requirement),
# so flip it on via the `--wandb.enable true` override on the training command below. Logs to
# each recipe's wandb entity/project (Nemo-automodel / automodel_convergence_runs). Requires
# WANDB_AUTOMODEL_API_KEY to have write access to that entity -- otherwise wandb.init() raises
# CommError on rank0 and strands the other ranks at the checkpoint-consolidation gloo barrier.
