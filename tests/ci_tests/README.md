...
0. **Source-load parity** (optional) -- With `check_source_load_parity: true`, run the checkpoint lifecycle with a pristine trainer, then load and release the raw HF source model, construct a parity-only trainer, and compare its pre-training logits against the HF logits as the final blocking phase
