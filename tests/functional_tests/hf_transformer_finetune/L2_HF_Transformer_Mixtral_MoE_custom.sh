...
# ── Step 1: Training smoke-test with ep_size=2 ───────────────────────
...
examples/llm_finetune/finetune.py \
    --config examples/llm_finetune/mistral/mixtral_2layer_moe_custom.yaml \
...
# ── Step 2: KL-divergence parity check (ep_size=1 vs ep_size=2) ─────
python -m torch.distributed.run --nproc_per_node=2 --nnodes=1 \
    tests/functional_tests/hf_transformer_finetune/run_mixtral_ep_kl_divergence.py \
    --kl-threshold 1e-4
