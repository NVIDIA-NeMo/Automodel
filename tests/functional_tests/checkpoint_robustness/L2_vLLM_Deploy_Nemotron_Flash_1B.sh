...
#!/bin/bash
# Requires: L2_Checkpoint_Robustness_Nemotron_Flash_1B.sh to have run first.
# Note: this needs to launch in an environment with vllm installed.
set -xeuo pipefail

export PYTHONPATH=${PYTHONPATH:-}:$(pwd)

SFT_CKPT_DIR="/adasif/checkpoints/robustness_nemflash1b_sft"
PEFT_CKPT_DIR="/adasif/checkpoints/robustness_nemflash1b_peft"
...
python -m pytest tests/functional_tests/checkpoint_robustness/test_checkpoint_vllm_deploy.py \
    ...
    --tokenizer nvidia/Nemotron-Flash-1B \
    --max_new_tokens 50 \
    --trust_remote_code

# Step 2: vLLM PEFT native LoRA
python -m pytest tests/functional_tests/checkpoint_robustness/test_checkpoint_vllm_deploy.py \
    --model_path nvidia/Nemotron-Flash-1B \
