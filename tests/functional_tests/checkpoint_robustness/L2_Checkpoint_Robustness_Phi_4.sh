...
SFT_CKPT_DIR="/adasif/checkpoints/robustness_phi4_sft"
PEFT_CKPT_DIR="/adasif/checkpoints/robustness_phi4_peft"
...
python -m torch.distributed.run --nproc_per_node=8 --nnodes=1 \
    ...
    --config examples/llm_finetune/phi/phi_4_squad.yaml \
    --model.pretrained_model_name_or_path microsoft/phi-4 \
...
python -m torch.distributed.run --nproc_per_node=8 --nnodes=1 \
    -m pytest tests/functional_tests/checkpoint_robustness/test_checkpoint_robustness_llm.py \
    --config examples/llm_finetune/phi/phi_4_squad_peft.yaml \
    --model.pretrained_model_name_or_path microsoft/phi-4 \
    --step_scheduler.max_steps 5 \
    --step_scheduler.global_batch_size 16 \
    --step_scheduler.local_batch_size 2 \
