...
SFT_CKPT_DIR="/adasif/checkpoints/robustness_baichuan_sft"
PEFT_CKPT_DIR="/adasif/checkpoints/robustness_baichuan_peft"
...
python -m torch.distributed.run --nproc_per_node=8 --nnodes=1 \
    ...
    --config examples/llm_finetune/baichuan/baichuan_2_7b_squad.yaml \
    --model.pretrained_model_name_or_path baichuan-inc/Baichuan2-7B-Chat \
...
python -m torch.distributed.run --nproc_per_node=8 --nnodes=1 \
    -m pytest tests/functional_tests/checkpoint_robustness/test_checkpoint_robustness_llm.py \
    --config examples/llm_finetune/baichuan/baichuan_2_7b_squad_peft.yaml \
    --model.pretrained_model_name_or_path baichuan-inc/Baichuan2-7B-Chat \
    --step_scheduler.max_steps 5 \
    --step_scheduler.global_batch_size 16 \
    --step_scheduler.local_batch_size 2 \
