...
#!/bin/bash
# Requires: L2_Checkpoint_Robustness_Llama3_2_3B.sh to have run first.
# Note: this needs to launch in an environment with vllm installed.
set -xeuo pipefail

export PYTHONPATH=${PYTHONPATH:-}:$(pwd)

SFT_CKPT_DIR="/adasif/checkpoints/robustness_llama_sft"
PEFT_CKPT_DIR="/adasif/checkpoints/robustness_llama_peft"
