...
# Step 1: SFT checkpoint robustness (TP=8 — 49B fits on 8x80GB)
python -m torch.distributed.run --nproc_per_node=8 --nnodes=1 \
    ...
    --config examples/llm_finetune/nemotron/llama3_3_nemotron_super_49B_squad.yaml \
    --model.pretrained_model_name_or_path nvidia/Llama-3_3-Nemotron-Super-49B-v1_5 \
...
# Step 2: PEFT checkpoint robustness (TP=2)
python -m torch.distributed.run --nproc_per_node=8 --nnodes=1 \
    -m pytest tests/functional_tests/checkpoint_robustness/test_checkpoint_robustness_llm.py \
    --config examples/llm_finetune/nemotron/llama3_3_nemotron_super_49B_squad_peft.yaml \
    --model.pretrained_model_name_or_path nvidia/Llama-3_3-Nemotron-Super-49B-v1_5 \
    --step_scheduler.max_steps 5 \
    --step_scheduler.global_batch_size 16 \
    --step_scheduler.local_batch_size 2 \
