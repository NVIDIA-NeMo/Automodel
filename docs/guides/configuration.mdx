...
- An existing `file.py:symbol` target loads that file directly without requiring an environment-variable opt-in. Loading either form executes the imported module's top-level code.
