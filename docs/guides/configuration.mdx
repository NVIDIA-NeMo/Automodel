...
The LLM and VLM fine-tuning recipes accept a `freeze_config:` section. Use `freeze_modules` and `unfreeze_modules` to select modules with the same fully qualified name glob patterns accepted by PEFT's `target_modules`:
...
Each matched module has all of its parameters updated recursively. Patterns match the complete module name, so use a leading wildcard when nesting can vary; for example, `*_proj` matches projection modules at any depth. A single string or a list of patterns is accepted.

Freeze selectors are applied first, followed by unfreeze selectors, so an unfreeze selector takes precedence when both match. Selection happens before distributed wrapping and optimizer construction. Under PEFT, LoRA parameters in modules that do not match a freeze selector retain their existing trainability, selected non-LoRA modules can be unfrozen, and non-LoRA parameters created during parallelization remain frozen.

The `freeze_vision_tower`, `freeze_audio_tower`, `freeze_language_model`, and `freeze_video_embedder` options are deprecated. Replace enabled options with the corresponding module name under `freeze_modules` and remove disabled options. For example:
...
Legacy-only configurations retain the existing implicit `freeze_vision_tower: true` default. A configuration containing `freeze_modules` or `unfreeze_modules` uses explicit-selector semantics and does not implicitly freeze vision modules; include a matching vision selector when migrating a configuration that relied on that default. Enabled legacy aliases retain their established attribute and substring matching before `unfreeze_modules` is applied.
