...
Only **strings** are translated by `translate_value()`:
...
FSDP2 uses the correctness-first `root` policy for fully frozen vision and audio towers and multimodal projectors:
...
## Model Attention and Hub Kernels

`model:` entries passed to `NeMoAutoModel*.from_pretrained` or `from_config` accept:

```yaml
model:
  _target_: nemo_automodel.NeMoAutoModelForCausalLM.from_pretrained
  pretrained_model_name_or_path: Qwen/Qwen2.5-7B
  force_hf: true                             # use Transformers dispatch
  attn_implementation: flash_attention_4    # installed implementation or Hub fallback
  use_kernels: true                         # optional Hub layer kernels
  use_liger_kernel: false                   # auto-disabled when use_kernels is true
```

`attn_implementation` and `use_kernels` are independent. Use
`flash_attention_2`, `flash_attention_3`, or `flash_attention_4`, or provide an
explicit Hub repo ID such as `kernels-community/flash-attn4`. This setting
selects attention only. `use_kernels` replaces norms, MLP, and linear layers
through Transformers `kernelize`. Non-`kernels-community` attention repos also need
`allow_all_kernels: true`. Install with `uv sync --extra hub_kernels`. Full
details: [Hub Kernels](/development/hub-kernels).
