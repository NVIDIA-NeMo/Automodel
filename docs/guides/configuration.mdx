...
- `str(cfg)` / `repr(cfg)` prints placeholders (for example, `${DATABRICKS_TOKEN}`), not resolved secrets.
...
model:
  ...
  attn_implementation: kernels-community/flash-attn2  # Hub flash attention
  use_kernels: true                                   # optional Hub layer kernels
  use_liger_kernel: false                             # auto-disabled when use_kernels is true
```

`attn_implementation` and `use_kernels` are independent. A Hub repo id selects
attention only. `use_kernels` replaces norms, MLP, and linear layers through Transformers
`kernelize`. Non-`kernels-community` attention repos also need
