...
`megatron_fsdp` uses Megatron Core MFSDP v2 and initially supports DP-only
ZeRO-3. Keep `zero_dp_strategy: 3` and leave TP, CP, PP, EP, and DP replication
at size one; use `fsdp2` for those topologies.
