...
| `activation_checkpointing_scope` | `str \| list[str]` | `"all"` | FSDP2/DDP strategy field that limits AC to model parts such as `language`, `vision`, `audio`, or `multimodal`. In Python, set it on `FSDP2Config` or `DDPConfig`; in YAML, place it under `distributed:`. See [Use Gradient (Activation) Checkpointing](/guides/gradient-checkpointing#scope-activation-checkpointing). |
