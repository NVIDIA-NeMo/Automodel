...
#### Linux on Debian-Based Systems (Ubuntu and Debian)
...
#### Linux on RHEL, CentOS, and Fedora
...
**Recommended: WSL2 and Docker**

Run NeMo AutoModel in WSL2 with Docker Desktop:

1. Install WSL2 and Docker Desktop.
2. Use a Docker container within WSL2 (follow the Linux instructions).

**Alternative: WSL2 and virtualenv**

Install directly in a WSL2 Ubuntu environment (follow the Debian instructions).
...
- **Storage**: At least 50 GB free space for models and datasets
...
Install from GitHub to access the latest features on the `main` branch or to contribute to the project.

#### Use `uv` with a Git Repo
```bash
...
| `hub_kernels` | Hugging Face `kernels` package (Hub flash attention / layer kernels) | No |
...
decode video or use OpenCV or Qwen vision preprocessing (Qwen2.5-VL, Qwen3-VL,
Qwen3-Omni, Mistral VLMs, and Nemotron-Omni) additionally require the
`vlm-media` extra (see [Media Extras](#media-extras-video-and-image-decode)):
...
### Media Extras (Video and Image Decode)

Media decode dependencies (OpenCV, decord, PyAV through the Qwen utils, and
