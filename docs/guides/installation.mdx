...
decode **video** or use **OpenCV / Qwen vision preprocessing** — Qwen2.5-VL,
Qwen3-VL, Qwen3-Omni, Mistral VLMs, and Nemotron-Omni — additionally require the
...
imageio-ffmpeg) bundle **FFmpeg** and are kept **opt-in** so the default install
and the shipped Docker image stay clear of FFmpeg CVEs. They are split by domain:
...
<Warning>
**`[all]` does NOT include the media extras, and the Docker image ships without
them** — deliberately, to keep FFmpeg out of the scanned image. After
`pip3 install nemo-automodel[all]` (or inside the container) add media
