...
description: "Prepare images and videos as cached latent files for NeMo AutoModel diffusion training, including processors, bucketing, and dataloader YAML."
position: 7
---
## Introduction
...
Place your images in a directory. Supported formats include `jpg`, `jpeg`, `png`, `webp`, and `bmp`.
...
Place your videos in a directory. Supported formats include `mp4`, `avi`, `mov`, `mkv`, and `webm`.
...
<Note>
Preprocessing requires the `diffusion-media` extra (it uses OpenCV). For a released-package environment, create and activate a virtual environment, then install it with `uv pip install "nemo-automodel[diffusion,diffusion-media]"`. In the Docker container, run `cd /opt/Automodel && uv pip install ".[diffusion-media]"` first. See [Media Extras](/get-started/installation#media-extras-video--image-decode).
...
| `wan` | Video | Wan 2.1 |
| `wan_animate2` | Video triplet | Wan-Animate-2 |
| `hunyuan` | Video | HunyuanVideo 1.5 |

For the Wan-Animate-2 triplet manifest format and preprocessing command, see
[Prepare Your Data](/recipes-e2e-examples/wan-animate-2#prepare-your-data).
...
Use video mode to encode the full video as a single sample. This mode is recommended for training:
...
Use frames mode to extract evenly spaced frames. Each frame becomes a separate sample:
...
|----------|-------------|
| `--processor` | Processor name (`flux`, `wan`, `wan_animate2`, `hunyuan`) |
| `--model_name` | Hugging Face model name (uses processor default if omitted) |
| `--output_dir` | Output directory for cached data |
| `--shard_size` | Number of samples per metadata shard (default: 10,000) |
...
| `--center_crop` / `--no_center_crop` | Enable or disable center cropping (default: enabled) |
...
- **Text embeddings**: Pre-computed embeddings from the model text encoder.
- **First frame**: Reference image for image-to-video conditioning (video mode only).
- **Image embeddings**: Embeddings for models that support i2v conditioning (video mode only).
- **Metadata**: Original and bucket resolutions, caption, and source path.
...
During preprocessing, the `--resolution_preset` argument controls the pixel budget used for bucketing. Available presets: `256p`, `512p`, `768p`, `1024p`, `1536p`. Alternatively, use `--max_pixels` for a custom pixel budget, or `--height` and `--width` to disable bucketing and use a fixed resolution.
...
- `SequentialBucketSampler`: Groups samples by resolution bucket.
- `dynamic_batch_size`: When `true`, adjusts the batch size per resolution bucket to maintain constant memory usage.

## YAML Configuration

### Video Dataloader (Wan 2.1 and HunyuanVideo)
