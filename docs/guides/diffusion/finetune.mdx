...
:::{important}
**Docker users:** Checkpoints are lost when the container exits unless you bind-mount the checkpoint directory to the host. See [Install with NeMo Docker Container](../installation.mdx#install-with-nemo-docker-container) and [Saving Checkpoints When Using Docker](../checkpointing.mdx#saving-checkpoints-when-using-docker).
:::
...
For the full set of arguments and input format details, see the [Diffusion Dataset Preparation](dataset.mdx) guide.
