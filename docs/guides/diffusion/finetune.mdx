...
Diffusion models generate images and videos by learning to reverse a noise process. They start from random noise and iteratively refine it into coherent visual output guided by a text prompt. Pretrained diffusion models, such as [FLUX.1-dev](https://huggingface.co/black-forest-labs/FLUX.1-dev) for images or [Wan 2.1](https://huggingface.co/Wan-AI/Wan2.1-T2V-1.3B-Diffusers) for video, produce impressive general-purpose results. However, they do not contain information about your particular visual domain, style, or subject matter. Fine-tuning bridges that gap by adapting the model to your data so that it produces outputs that match your requirements, without the cost of training from scratch.

NeMo AutoModel uses [flow matching](https://arxiv.org/abs/2210.02747), a modern generative framework that learns to transform noise into data by regressing a velocity field along straight interpolation paths. It integrates with [Hugging Face Diffusers](https://huggingface.co/docs/diffusers) to provide distributed fine-tuning for text-to-image and text-to-video models. This guide walks you through the process from installation through training and inference, using [Wan 2.1 T2V 1.3B](https://huggingface.co/Wan-AI/Wan2.1-T2V-1.3B-Diffusers) as a running example.
...
| **2. Prepare Data** | [Prepare Your Dataset](#prepare-your-dataset) | Encode raw images and videos into `.meta` latent files |
...
Diffusion fine-tuning requires the `diffusion` extra and `diffusion-media` (OpenCV for image and video preprocessing and imageio-ffmpeg for T2V export):
