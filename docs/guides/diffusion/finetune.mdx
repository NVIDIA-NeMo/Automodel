...
Timesteps and noise are drawn under a fixed seed, and the training RNG state is restored afterwards,
so the curve reflects the model rather than the sampling and enabling validation does not change the
training trajectory.
