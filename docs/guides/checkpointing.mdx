...
`msc://` paths use NVIDIA MultiStorageClient (MSC) object storage. NeMo AutoModel supports them for sharded Distributed Checkpoint (DCP) model and optimizer data when `checkpoint.save_consolidated` is `false`. However, a full recipe checkpoint is not self-contained in object storage: RNG, dataloader, and other recipe state written with `torch.save`, plus the `LATEST` and `LOWEST_VAL` pointers, remain local to the training environment. The interrupted-save marker and automatic stale-directory replacement described above also require a local filesystem and are unavailable for `msc://` roots.
...
```yaml
checkpoint:
  checkpoint_dir: msc://bucket/path
