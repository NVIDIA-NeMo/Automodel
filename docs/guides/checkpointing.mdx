...
A save can be cut short by a wall-clock limit, a preemption, or an out-of-memory error, which leaves a partially written `epoch_<E>_step_<S>` directory behind. NeMo AutoModel marks each checkpoint directory as in progress while it is being written and clears the mark only after every component is saved and the checkpoint is published. Directories left by an interrupted save therefore stay marked, which determines how they are treated:

- **Resume skips them.** `LATEST` is advanced only at publication, and both the pointer lookup and the step-order fallback ignore marked directories. Training resumes from the most recent complete checkpoint. Setting `checkpoint.restore_from` to a different pointer, such as `LOWEST_VAL`, whose target is marked raises an error instead of silently restoring a different checkpoint than you asked for.
- **The next save of that step replaces them.** Checkpoint directory names are derived from the step, so the resumed run targets the same directory when it reaches that step again. NeMo AutoModel removes the leftover directory, logs a warning, and writes the new checkpoint in its place.
- **Retention does not preserve them.** When `max_recent_checkpoints` is set, a marked directory never occupies a slot in the recent window.

<Warning>

Replacement applies to any existing checkpoint directory, not only to marked ones. If you resume from an older checkpoint with `restore_from` and training runs past a step that already has a complete checkpoint, that checkpoint is removed and rewritten with the recomputed state. NeMo AutoModel logs a warning that distinguishes the two cases. Copy checkpoints you need to keep to a separate directory, or set a different `checkpoint.checkpoint_dir` for the new run.

</Warning>

<Note>

Asynchronous saves are published on the following save call, so a job that stops in between leaves the last checkpoint marked as in progress even though its files are complete. Resume falls back to the previous published checkpoint in that case.

</Note>

<Warning>

`checkpoint.checkpoint_dir` must be a local or shared filesystem path. Setting it to an `msc://` remote storage URI raises `ValueError` at startup. Training checkpoints write RNG, dataloader, and recipe metadata with `torch.save`, and publish the `LATEST` and `LOWEST_VAL` pointers as local symbolic links; neither works on object storage, so a remote root fails partway through a save. Loading a base model from remote storage is unaffected.
