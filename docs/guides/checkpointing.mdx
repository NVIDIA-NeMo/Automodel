...
Using shared clusters, jobs can be preempted or reach their wall-time limits
...
exit gracefully. That way training can resume from the moment of preemption
...
- A signal number (e.g. `15`).
- A list of any of the above to watch multiple signals at once, for example
  `["SIGUSR1", "SIGTERM"]`.
...
so every rank observes the preemption at the same step. All the ranks write
...
delivers the warning signal to the batch script via the `--signal` directive:
...
processes. The workers must be signalled directly rather than `srun` or
...
batch node alone is sufficient — the all-gather propagates the flag to every
rank on every node.
...
> **Note:** `--signal=B:USR1@<sec>` guarantees the warning before a
> *wall-time* termination. Whether the same warning is delivered on a
> *scheduler preemption* (e.g. a higher-priority job taking the nodes) depends
> on the cluster's Slurm configuration (`GraceTime` on the partition and
> `PreemptParameters=send_user_signal`). Ask your cluster administrator if
> preemption-triggered signals are required.
