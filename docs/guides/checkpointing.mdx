...
Most custom AutoModel implementations need a state dict adapter to convert HF weight names or tensor layouts. Models whose weights already match, such as Llama, Qwen2, and Qwen3, can omit it. See the [model-onboarding guide](https://github.com/NVIDIA-NeMo/Automodel/blob/main/skills/nemo-automodel-model-onboarding/SKILL.md) for implementation and validation guidance.
...
- A list of preceding options to watch multiple signals at once, such as
  `["SIGUSR1", "SIGTERM"]`.
- `null` to disable preemption handling entirely.

By default, `SIGTERM` is watched, so runs already checkpoint on a graceful
...
for the signal to reach a single rank. For example, the signal can reach only
the workers on one node.
...
time to cover one step and the time to write a checkpoint for your model
