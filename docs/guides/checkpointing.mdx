...
- **Resume skips them.** `LATEST` is advanced only at publication, and the step-order fallback used when no `LATEST` pointer exists ignores marked directories. Training resumes from the most recent complete checkpoint.
