...
The schedule values above are taken from the config shipped with the authors' released
`scdd_pu_0.1` checkpoint. Their only other released setting differs solely in `uniform_ratio: 0.2`,
so that is the first knob to sweep.
