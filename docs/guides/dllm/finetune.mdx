...
every visible position on top of the familiar denoising term at `[MASK]` positions. That extra term
...
only other released setting differs solely in `uniform_ratio: 0.2`, so that is the first hyperparameter to
sweep.
...
model:
  ...
  torch_dtype: float32          # fp32 master weights; compute is bf16 using mp_policy
  canvas_length: 256
...
[arXiv:2605.18810](https://arxiv.org/abs/2605.18810). Their weighted sum is
divided by the globally reduced batch size times `num_blocks_per_sample`. This
is equivalent to the local mean used by the dedicated DFlash recipe
(`train_dflash.py`), including across data-parallel ranks and gradient
accumulation, so `dpace_alpha` and `optimizer.lr` are portable between paths.

Switching `loss_type` from `dflash` to a `dpace*` variant here does change the
scale: `dflash` uses `normalize="tokens"` (a supervised-token count) in this
strategy, not `normalize="mean"`, so it is on a different scale than either
D-PACE convention. Retune `optimizer.lr` when switching `loss_type`.
