...
The sampler selected with `--sampler` supplies these preset values. LLaDA uses the standalone AutoModel sampler. LLaDA2 and Nemotron call their models' built-in `generate()` methods. `--remasking` and `--no_kv_cache` do not affect LLaDA2 generation.
