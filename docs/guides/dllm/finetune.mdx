...
Diffusion language models (dLLMs) generate text by iteratively denoising masked tokens rather than generating one token at a time from left to right, as autoregressive (AR) models do. Starting from a sequence of `[MASK]` tokens, the model progressively unmasks the most confident positions over multiple denoising steps until the full response is revealed.
...
- **LLaDA or LLaDA2**: These bidirectional masked diffusion language models (MDLMs) receive corrupted tokens and predict the clean token at each masked position (see the [LLaDA2 paper](https://arxiv.org/abs/2602.08676)).
...
| DFlash | `dflash` | Decay-weighted cross-entropy (Eq. 4) | Speculative block decoding (draft and target) | [dflash_sft.yaml](https://github.com/NVIDIA-NeMo/Automodel/blob/main/examples/dllm_sft/dflash_sft.yaml) |
...
| `hybrid` | `HybridStrategy` | Nemotron-Labs-Diffusion-style: model receives clean tokens and `masked_indices`, with a combined diffusion and AR loss |
| `dflash` | `DFlashStrategy` | DFlash: frozen target LM provides hidden states. Draft model is trained with decay-weighted loss |
...
| `dllm.block_size` | When set, uses blockwise corruption (otherwise uniform). Hybrid mode only |
...
| `draft_acc` | Overall fraction of valid block positions where `argmax(draft_logits) == target_token` | Console line, Weights & Biases, MLflow, Comet, and file logger |
| `draft_acc_k{k}` | Same fraction restricted to block offset `k` (`k = 1..block_size-1`), which forms the acceptance-length curve | Weights & Biases, MLflow, Comet, and file logger (one panel per offset). Intentionally omitted from the console line to keep it readable |
