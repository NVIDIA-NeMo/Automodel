...
The generation script ([`generate.py`](https://github.com/NVIDIA-NeMo/Automodel/blob/main/examples/dllm_generate/generate.py)) supports chat and raw generation. Pick the sampler that matches the trained family with `--sampler {llada,llada2,nemotron}`. Infilling is also available for LLaDA and Nemotron, but is not supported by the LLaDA2 generation path.
...
The sampler selected with `--sampler` supplies these preset values. LLaDA uses the standalone AutoModel sampler. LLaDA2 and Nemotron call their models' built-in `generate()` methods. `--remasking` and `--no_kv_cache` do not affect LLaDA2 generation. LLaDA2-specific `--editing_threshold` and `--max_post_steps` control post-generation refinement.
...
| `--editing_threshold` | LLaDA2 confidence threshold for revising committed tokens | Not used | 0.0 | Not used |
| `--max_post_steps` | Maximum LLaDA2 post-generation refinement steps | Not used | 16 | Not used |
