...
dflash:
  ...
  loss_type: dflash                             # dflash | dpace | dpace-cumulative-confidence-only | dpace-continuation-value-only
  dpace_alpha: 0.5                              # smoothing alpha for the D-PACE variants
```
...
| `dflash.loss_type` | `dflash` (decay-weighted CE, default) or a D-PACE variant: `dpace`, `dpace-cumulative-confidence-only`, `dpace-continuation-value-only` (detached confidence-aware weights) |
| `dflash.dpace_alpha` | Smoothing α in `[0, 1]` for the D-PACE confidence products; ignored when `loss_type: dflash` |

<Note>
The `dpace*` variants implement D-PACE (Dynamic Position-Aware Cross-Entropy),
which reweights each position by detached cumulative draft confidence; see
[arXiv:2605.18810](https://arxiv.org/abs/2605.18810).
</Note>
...
|--------|---------|-------|
| `draft_acc` | Overall fraction of valid block positions where `argmax(draft_logits) == target_token` | Console line and wandb / mlflow / comet and file logger |
| `draft_acc_k{k}` | Same fraction restricted to block offset `k` (`k = 1..block_size-1`), the acceptance-length curve | wandb / mlflow / comet and file logger (one panel per offset); intentionally omitted from the console line to keep it readable |
...
## Fine-Tune the Model
Run the following commands to launch fine-tuning for each supported model family.
