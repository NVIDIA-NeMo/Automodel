...
divided by the batch size times `num_blocks_per_sample`, the same convention
the dedicated DFlash recipe (`train_dflash.py`) uses, so `dpace_alpha` and
`optimizer.lr` are portable between the two training paths. This makes the
reported loss smaller than the published D-PACE value by the block count,
which is a constant and so leaves the objective unchanged.
