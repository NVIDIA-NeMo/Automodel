...
| DiffusionGemma | `block_diffusion` | Flat block-diffusion cross-entropy + encoder AR | Built-in HF diffusion sampler (entropy-bounded denoising) | [diffusion_gemma_sft.yaml](https://github.com/NVIDIA-NeMo/Automodel/blob/main/examples/dllm_sft/diffusion_gemma_sft.yaml) |
| DFlash | `dflash` | Decay-weighted cross-entropy (Eq. 4) | Training only (decoding lives in the speculative-decoding stack) | [dflash_sft.yaml](https://github.com/NVIDIA-NeMo/Automodel/blob/main/examples/dllm_sft/dflash_sft.yaml) |

DiffusionGemma fine-tuning (full and LoRA, with expert parallelism for its 26B-A4B MoE) has a dedicated walkthrough: see the [DiffusionGemma Fine-Tuning Guide](/recipes-e2e-examples/diffusiongemma).
...
| `block_diffusion` | `BlockDiffusionStrategy` | DiffusionGemma-style: uniform random-token corruption over a response canvas, flat cross-entropy + co-trained encoder AR loss |
...
The `--checkpoint` argument accepts several path types, such as a path to a `consolidated/` directory, a step directory (for example, `.../epoch_0_step_499`), or the top-level checkpoint directory. The script automatically follows the path to `LATEST/model/consolidated/`. Training with the shipped example configs writes this consolidated HF-format directory automatically at the final checkpoint (`checkpoint.save_consolidated: final`), so the directory printed at the end of training can be passed to `--checkpoint` directly.
