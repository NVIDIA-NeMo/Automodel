...
| Field | Description |
|-------|-------------|
| `dllm.mode` | Training strategy (`mdlm`, `hybrid`, `block_diffusion`, `dflash`, or `idlm`) |
...
| `dllm.vocab_size` | Vocabulary size for uniform-random token corruption. Required for `block_diffusion` (`262144` for DiffusionGemma). |
...
[arXiv:2605.18810](https://arxiv.org/abs/2605.18810). They are normalized to the
same scale as `dflash` (the weighted sum is divided by the batch size times the
sampled block count), so switching `loss_type` needs no change to `optimizer.lr`.
Note that this makes the reported loss smaller than the published D-PACE value by
the block count, which is a constant and so leaves the objective unchanged.
</Note>
