...
SCDD ([arXiv:2603.02230](https://arxiv.org/abs/2603.02230)) trains self-correction into the model
...
The SCDD loss upcasts logits to fp32 and holds roughly two extra logits-sized tensors while it sums
over the vocabulary. Scale sequence length with FSDP2 sharding and a smaller local batch rather than
by growing the micro-batch.
...
| `dllm.vocab_size` | Vocabulary size, including `[MASK]`. Required by `scdd` and `block_diffusion`, whose corruption draws replacement tokens over the vocabulary. |
...
```bash
uv run automodel finetune dllm \
