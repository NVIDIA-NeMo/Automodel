...
- See also: [LLM datasets](llm/dataset.mdx), [VLM datasets](vlm/dataset.mdx), and [Retrieval dataset](llm/retrieval-dataset.mdx) for deeper, task-specific guides.
...
dataset:
  ...
  start_of_turn_token: "<|assistant|>"
```
See the detailed guide, [Column-Mapped Text Instruction Dataset](llm/column-mapped-text-instruction-dataset.mdx), for more information.
...
```
See the [Function Calling guide](llm/toolcalling.mdx) for an end-to-end example with FunctionGemma.
...
collate_fn:
  ...
  p_max_len: 512
```
See the detailed guide, [Retrieval dataset](llm/retrieval-dataset.mdx), for more information.
...
```
See the detailed [pretraining guide](llm/pretraining.mdx), which uses MegatronPretraining data.
...
See [Gemma-3n](omni/gemma3-3n.mdx) and [VLM dataset](vlm/dataset.mdx) for end-to-end examples.
...
See the [Diffusion Dataset Preparation](diffusion/dataset.mdx) guide for full preprocessing instructions and configuration details.
