...
- If a dataset you need is missing, open a [GitHub issue](https://github.com/NVIDIA-NeMo/Automodel/issues) with a short description and an example schema so the team can prioritize support.
...
- Use case: instruction or QA tuning with either prompt-and-answer formatting or chat-template formatting
...
- You can use the optional `seq_length` parameter for padding or truncation.
...
- Key args:
  ...
  - `mask_generation_prompt`: optionally exclude the template-supplied prefix of each assistant turn (the role header and any empty reasoning block such as `<think></think>`) from loss
  - `skip_invalid_samples`: if `true`, skip malformed JSONL lines when reading local files (warnings log skip counts); default `false` fails fast on a bad line
<Note>
- Requires a tokenizer with chat template support
- Supports both single-turn and multi-turn tool calling
...
- Set `mask_generation_prompt: true` when fine-tuning a hybrid-reasoning model (Nemotron, Qwen3, GLM, ...) on non-thinking data. Those chat templates insert an empty reasoning block (for example `<think></think>`) into every assistant turn that has no `reasoning_content`; at inference the generation prompt supplies that block, so training on it only teaches the model to close the block immediately. The option masks the role header and that block for every assistant turn. It is detected by rendering the template's generation prompt, so it works for any template without hardcoded tag strings
...
<Note>
- Streams contiguous `seq_len` slices and supports optional BOS alignment and `.bos.idx` sidecar files
...
- **Shopify product catalogue (product image to taxonomy category)**: `nemo_automodel.components.datasets.vlm.datasets.make_shopify_product_catalogue_dataset` (HF: `Shopify/product-catalogue`)
- **CommonVoice 17 (CV17) (audio)**: `nemo_automodel.components.datasets.audio.datasets.make_cv17_dataset`

RDR and CORD-V2 return lists. MedPix, Shopify product catalogue, and CommonVoice 17 return Hugging Face `Dataset` objects with lazy transforms. Each sample follows the conversation schema expected by `apply_chat_template`, e.g.:
```python
{
  "conversation": [
    {
