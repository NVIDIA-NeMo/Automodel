...
| --- | --- | --- |
| `model.dtype` | Storage dtype of the sharded parameter that PyTorch holds. | For `torch.optim.Adam`/`AdamW`, the EMA buffers (`exp_avg`, `exp_avg_sq`) use this dtype. TE FusedAdam configures master weights and EMA dtypes separately. |
...
| torch AdamW with `model.dtype: float32` | fp32 | fp32 master weight (the resident param) and fp32 Adam EMA buffers | Robust starting point for new or precision-sensitive models — params, gradients, and optimizer state are all fp32. Trade-off: writes fp32 training checkpoints. |
...
With bf16 resident parameters, [TE FusedAdam](https://github.com/NVIDIA/TransformerEngine/blob/main/transformer_engine/pytorch/optimizers/fused_adam.py) can preserve fp32 master weights while storing both Adam EMA buffers in bf16. This halves the storage for those two buffers compared with fp32 moments:
