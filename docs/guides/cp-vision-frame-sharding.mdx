...
The replicated path is used only when both the merged visual-token count is below
`min_tokens` and the image/frame count is below `min_frames`. The frame threshold
