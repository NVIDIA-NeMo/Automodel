...
vision-tower blocks. Vision-tower checkpointing behaves the same as on the
generic path and works with any attention backend; no additional configuration
is needed. Frozen vision towers stay out of activation checkpointing on both
paths.
</Note>
