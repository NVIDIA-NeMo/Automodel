...
```yaml
# Add this field to the `distributed` section of your recipe config.
...
...
<Note>
Expert-parallel MoE configs (`ep_size > 1`) use the MoE parallelizer's
activation checkpointing path, which respects the same
`activation_checkpointing_scope` field with the same semantics: `all` (the
default) checkpoints the text/MoE decoder blocks plus the trainable VLM vision
tower, `language` the decoder blocks only, and `vision` only the trainable
vision-tower blocks. Vision-tower blocks get the same per-submodule wrapping
(attention/MLP/norms) as the generic path, and the SDPA backend set active at
checkpoint-forward time is automatically snapshotted and restored during the
backward-time recompute, so ambient backend forcing that does not span the
recompute cannot fault the replay. Frozen vision towers stay out of activation
checkpointing on both paths.
</Note>
