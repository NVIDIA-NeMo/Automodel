...
The legacy `force_hf=True` option is equivalent to `tokenizer_backend="transformers_auto"`.
`force_default=True` bypasses registered NeMo tokenizers while remaining on the wrapped fallback route.
Contradictory route selections raise `ValueError` instead of silently choosing one.
