...
The `packed_sequence` block accepts these settings:
...
- **max_packs**: Optionally caps how many packed samples are created.
- **drop_long_samples**: For `neat`, drops samples longer than `packed_sequence_size` when `true`; otherwise the packer raises an error.
- **num_proc**: Number of worker processes used to tokenize the dataset before packing. Packing first runs one full pass over the dataset, which tokenizes every sample serially; setting `num_proc > 1` front-loads that tokenization across processes (identical, order-preserving output). Defaults to `1` (serial). The speedup grows with per-sample tokenization cost (long multi-turn chats, heavy chat templates) and available CPU cores. This runs independently on each data-parallel rank, so size it so `local_ranks * num_proc` does not oversubscribe the node's CPUs.
