...
|---------------------------|---------------------|------------------------|----------------------------|
| `llama`, `llama_bidirec` | Uses `LlamaBidirectionalModel` with configurable text attention. | Uses `LlamaBidirectionalForSequenceClassification` | Bi-encoder: `pooling`, wrapper-level `l2_normalize`, `is_causal`, and top-level recipe `temperature`. Cross-encoder: `pooling`, `num_labels`, and `temperature` on the Llama scoring config. |
| `ministral3` | Uses the stock Hugging Face `Ministral3Model` with configurable text attention. | Uses the stock Hugging Face `Ministral3ForSequenceClassification` | Bi-encoder: wrapper-level `pooling`, `l2_normalize`, `is_causal`, plus top-level recipe `temperature`. Cross-encoder: forwards `num_labels`; custom `pooling` and `temperature` are ignored. |
...
| Any model type without a retrieval registry entry | Falls back to `AutoModel` and applies the resolved attention policy. | Falls back to `AutoModelForSequenceClassification` | Bi-encoder fallback receives standard Hugging Face `from_pretrained` keyword arguments. `pooling`, `l2_normalize`, and `is_causal` apply in the NeMo AutoModel wrapper. Cross-encoder fallback forwards only `num_labels`. Custom `pooling` and `temperature` are ignored. |

For bi-encoders, `model.is_causal` selects text self-attention. An explicit YAML or API value takes precedence over a
saved `is_causal` value in the checkpoint's text config; when neither exists, the model defaults to `false` for
bidirectional retrieval. The resolved value is saved with the text config. Cross-encoder attention is unchanged.
For composite models such as Llama Nemotron VL, this setting affects only the language tower and does not change vision
attention.
...
bi-encoder applies the resolved `is_causal` value to the text config and attention modules, while registered retrieval
backbones such as the Llama path provide retrieval-specific implementations. Confirm that an unregistered architecture
honors the selected policy before relying on it for retrieval.
