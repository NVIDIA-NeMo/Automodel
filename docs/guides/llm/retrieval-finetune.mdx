...
- Run the example commands from a source checkout or an NGC container workspace that contains the repository
  ...
  without the repository, copy the referenced config or script files into your own project and update the paths.
...
time and disk. For a tiny data-loading test, point the config at a small local retrieval JSON or JSONL sample.

The first artifact to check is `training.jsonl` under `checkpoint.checkpoint_dir`. JSONL metrics are buffered, so
stdout or stderr are still the best live signal during a very short run.
...
| Any other Hugging Face model type | Falls back to `AutoModel` | Falls back to `AutoModelForSequenceClassification` only when the model type is not listed above | Bi-encoder fallback receives standard Hugging Face `from_pretrained` kwargs, and `pooling` and `l2_normalize` still apply in the AutoModel wrapper. Cross-encoder fallback forwards `num_labels`. Do not assume custom `pooling` or `temperature` are accepted unless that HF class documents them. |
...
|-----------|----------|-------|
| `hf://` AutoModel retrieval schema | You want a tutorial run or shared public dataset | Requires an AutoModel-style Hugging Face subset with a companion corpus split. |
...
| Normalized Arrow | You are preparing a full-scale or image-heavy vision-language retrieval dataset | Stores query rows and referenced corpus documents in portable local Arrow shards, avoiding repeated image-corpus loading and preparation in GPU jobs. |

<Warning>
Do not start full-scale VL training directly from unprepared raw sources. Loading a large image corpus and building its
dataset cache can substantially delay the first training step while the GPUs are already allocated. Across a multi-GPU
job, that startup can waste GPU-hours before optimization begins. Run the normalized dataset preparation script on CPU
nodes first, then train from the generated Arrow dataset.

</Warning>

For typical text-only retrieval, the original `make_retrieval_dataset` path is usually sufficient. If its initial startup
is slow, use the CPU cache-warming script in the
[retrieval data preparation tools](https://github.com/NVIDIA-NeMo/Automodel/blob/main/tools/retrieval/README.md) without
changing the dataset configuration. Normalized Arrow remains available for unusually large text corpora or when you
need a portable dataset. The same guide includes local and Slurm preparation commands and a storage comparison. Both
tools support text-only and vision-language retrieval.

The key field requirements differ by raw source:
...
| `hf://` AutoModel schema | `question` | non-empty `pos_doc`. The `neg_doc` field is optional in the source but required before training with negatives |
...
code block is an excerpt. Keep the scheduler, optimizer, checkpoint, and distributed sections from the full config or one
of the examples.
...
The following parameters are important for your bi-encoder configuration:
...
- `do_distributed_inbatch_negative`: optional model setting that treats passages from other data-parallel ranks as
  ...
  for pure data-parallel or FSDP2 retrieval runs (`tp_size: 1`, `cp_size: 1`). Same-document masking requires `doc_id`
  fields from corpus-backed or custom datasets. Inline JSONL does not provide duplicate-document masking. For
  multi-positive queries expanded into separate rows, keep it disabled unless your sampler or masking prevents sibling
  positives from becoming negatives. Keep it disabled for ColBERT-style pooling.
...
scores each query-passage pair. This code block is an excerpt. Keep the same scheduler, optimizer, checkpoint, and
...
The following parameters are important for your cross-encoder configuration:
...
Current retrieval datasets are map-style datasets loaded in each process, not streaming distributed inputs. Before a
multi-node run, either populate the Hugging Face cache on each node or place it on storage available to every node. Also
budget CPU RAM and local disk per rank for corpus-backed datasets.
...
stable query IDs, qrels keyed by those IDs, the query and passage prefixes and max lengths, and the K values to report.
...
Use stdout or stderr as the live per-step signal today. Watch `loss`, `grad_norm`, learning rate, GPU memory, and step time
before scaling to a longer run. On preempted or timed-out jobs, recent buffered JSONL metrics may be missing even when
stdout or stderr showed them.
...
For a full training, mining, and retraining loop, mine each desired restored subset separately, give each mined output and
...
PEFT or LoRA runs save adapter artifacts under the checkpoint `model` directory instead of the full consolidated export
...
first produce a Hugging Face-loadable merged or exported encoder with your adapter workflow and point
...
| OOM at startup or first batch | Lower `local_batch_size`, `q_max_len`, `p_max_len`, or `rerank_max_length`, and use LoRA for larger backbones. |
...
| `training.jsonl` does not update during a smoke test | Use stdout or stderr for live monitoring. JSONL metrics are buffered before flush. |
