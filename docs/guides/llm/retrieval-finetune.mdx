...
job, that startup can waste GPU-hours before optimization begins. Run the CPU Slurm array preparation first, then train
from its normalized Arrow output.
</Warning>
...
need a portable dataset. The same guide includes the normalization command, source-parallel Slurm launcher, and storage
comparison. Both tools support text-only and vision-language retrieval.
