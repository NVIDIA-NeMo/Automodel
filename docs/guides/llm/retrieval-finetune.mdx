...
| Normalized Arrow | You are preparing a full-scale or image-heavy vision-language retrieval dataset | Stores query rows and referenced corpus documents in portable local Arrow shards, avoiding repeated image-corpus loading and materialization in GPU jobs. |

<Warning>
Do not launch a cold full-scale VL training job directly against raw sources. Loading and materializing an image-heavy
corpus can substantially delay the first training step while the GPUs are already allocated. Across a multi-GPU job,
that startup can waste GPU-hours before optimization begins. Run the CPU Slurm array preparation first, then train from
its normalized Arrow output.
</Warning>

For typical text-only retrieval, the original `make_retrieval_dataset` path is usually sufficient. Normalized Arrow is
still available for unusually large text corpora or when you need a portable prepared artifact. If you need to keep the
original dataset configuration unchanged and train on the same cluster, you can instead warm its shared Hugging Face
cache on CPU nodes. See the
[retrieval data preparation tools](https://github.com/NVIDIA-NeMo/Automodel/blob/main/tools/retrieval/README.md) for the
normalization command, source-parallel Slurm launcher, cache-warming alternative, and storage tradeoffs. Despite `vl`
in some tool filenames, both preparation paths also support text-only retrieval.
