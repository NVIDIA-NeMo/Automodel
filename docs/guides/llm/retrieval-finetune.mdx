...
- Use a NeMo AutoModel environment with the full GPU training dependencies installed. For multi-GPU runs, use the NGC
...
|---------------------------|---------------------|------------------------|----------------------------|
| `llama`, `llama_bidirec` | Uses `LlamaBidirectionalModel` with configurable text attention. | Uses `LlamaBidirectionalForSequenceClassification` with configurable text attention. | Bi-encoder: `pooling`, wrapper-level `l2_normalize`, `is_causal`, and top-level recipe `temperature`. Cross-encoder: `pooling`, `num_labels`, `temperature`, and `is_causal` on the Llama scoring config. |
| `ministral3` | Uses the stock Hugging Face `Ministral3Model` with configurable text attention. | Uses the stock Hugging Face `Ministral3ForSequenceClassification` with configurable text attention. | Bi-encoder: wrapper-level `pooling`, `l2_normalize`, `is_causal`, plus top-level recipe `temperature`. Cross-encoder: forwards `num_labels` and applies `is_causal`; custom `pooling` and `temperature` are ignored. |
| `ministral3_bidirec` | Uses the legacy custom `Ministral3BidirectionalModel` with configurable text attention. | Direct cross-encoder scoring is not supported by the custom retrieval registry. | Bi-encoder: `pooling`, wrapper-level `l2_normalize`, `is_causal`, and top-level recipe `temperature`. |
| `llama_nemotron_vl` | Uses `LlamaNemotronVLModel` and applies the attention policy only to its language tower. | Direct cross-encoder scoring is not supported by the custom retrieval registry today. | Bi-encoder: `pooling`, wrapper-level `l2_normalize`, `is_causal`, and top-level recipe `temperature`. |
| Any model type without a retrieval registry entry | Falls back to `AutoModel` and applies the resolved attention policy. | Falls back to `AutoModelForSequenceClassification` and applies the resolved attention policy. | Bi-encoder fallback receives standard Hugging Face `from_pretrained` keyword arguments. `pooling`, `l2_normalize`, and `is_causal` apply in the NeMo AutoModel wrapper. Cross-encoder fallback forwards `num_labels` and applies `is_causal`. Custom `pooling` and `temperature` are ignored. |

For both retrieval wrappers, `model.is_causal` selects text self-attention. An explicit YAML or API value takes
precedence over a saved `is_causal` value in the checkpoint's text config. When neither exists, a bi-encoder defaults to
`false` for bidirectional retrieval, while a cross-encoder preserves the scoring backbone's native attention mode. The
resolved value is saved with the text config. For composite models such as Llama Nemotron VL, the setting affects only
the language tower and does not change vision attention.

Changing this setting changes the resulting embeddings. Regenerate stored corpus embeddings before querying an index
with a model that uses a different attention policy.
...
Treat unregistered decoder-only fallback models as an architecture experiment, not just a drop-in model swap. The
retrieval wrapper applies the resolved `is_causal` value to the text config and attention modules, while registered
retrieval backbones such as the Llama path provide retrieval-specific implementations. Confirm that an unregistered
architecture honors the selected policy before relying on it for retrieval.
...
|-----------|----------|-------|
| `hf://` NeMo AutoModel retrieval schema | You want a tutorial run or shared public dataset | Requires a NeMo AutoModel-style Hugging Face subset with a companion corpus split. |
...
| `hf://` NeMo AutoModel schema | `question` | Non-empty `pos_doc`. `neg_doc` is optional in the source but required before training with negatives. |
...
already follow the NeMo AutoModel retrieval schema, such as:
...
- `model.is_causal`: optional text self-attention policy. An explicit value overrides saved metadata; when omitted, the
  wrapper restores a saved value or preserves the scoring backbone's native mode.
...
NeMo AutoModel does not currently provide a one-command full-corpus retrieval evaluator in this guide. Use your existing
...
Retrieval recipes support the same parameter-efficient fine-tuning (PEFT) block used by other NeMo AutoModel recipes.
...
The mining script loads a concrete Hugging Face model path. It does not resolve NeMo AutoModel's `LATEST` keyword or pointer
...
NeMo AutoModel does not provide a mined-negative audit or cleanup utility. Before training, verify the output:
...
With `save_consolidated: true` and full fine-tuning, NeMo AutoModel also writes a Hugging Face-compatible model under the
following path:
...
`LATEST` is a resolver keyword: NeMo AutoModel follows the symlink or pointer file and can fall back to the highest
`epoch_*_step_*` checkpoint directory if the pointer is not usable. An explicit restore target, including `LATEST`,
is authoritative: NeMo AutoModel warns about an incompatibility but still restores the selected checkpoint. If
`checkpoint.restore_from` is omitted, NeMo AutoModel selects the latest candidate and resumes only when that candidate is
...
| Run resumes unexpectedly | Use a new or empty `checkpoint_dir`. NeMo AutoModel auto-detects compatible checkpoints when `restore_from` is omitted. |
