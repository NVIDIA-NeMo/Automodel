...
| Normalized Arrow | You want to prepare a large text or vision-language retrieval dataset once and reuse it across training jobs | Stores query rows and referenced corpus documents in portable local Arrow shards, avoiding repeated source loading and corpus materialization in GPU jobs. |

For full-scale training, normalized Arrow is the recommended prepared-data path. If you need to keep the original
`make_retrieval_dataset` configuration unchanged and train on the same cluster, you can instead warm its shared Hugging
Face cache on CPU nodes. See the
