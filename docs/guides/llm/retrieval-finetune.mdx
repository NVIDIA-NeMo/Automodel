...
For DDP recipes, set `distributed.static_graph: false` whenever
`gradient_accumulation_steps > 1`. If a nightly or release override reduces the GPU count without changing the global
and local batch sizes, it must also override `distributed.static_graph: false`. You may enable the static-graph
optimization only for a no-accumulation topology after verifying that the forward graph does not change between steps.
...
- Llama-Nemotron-Embed-VL-1B multimodal (visual document retrieval) example, including one topology-neutral optimized
  DDP config and documented batch/checkpointing overrides:
  [examples/retrieval/bi_encoder/nemotron_vl_1b/README.md](https://github.com/NVIDIA-NeMo/Automodel/blob/main/examples/retrieval/bi_encoder/nemotron_vl_1b/README.md)
