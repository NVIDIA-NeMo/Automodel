...
- **Repeat behavior**: `repeat_on_exhaustion=True` is currently the only supported mode, so the dataset restarts after the underlying stream is exhausted. Single-pass mode is not supported.
...
| `repeat_on_exhaustion`  | `True`  | Restart the stream on exhaustion (useful for step-based training). `True` is currently the only supported value; `False` raises `ValueError`. |
