...
## Prepared Normalized Arrow Bundles for VL Retrieval

For full-scale or image-heavy vision-language retrieval, prepare the raw sources on CPU nodes as a portable normalized
Arrow bundle before starting GPU training. The bundle keeps query rows as document-ID references and stores each
referenced document or image once in local corpus shards. This avoids spending allocated GPU time rebuilding Hugging
Face dataset and image-corpus state before training begins.

For typical text-only retrieval, the original `make_retrieval_dataset` factory is usually sufficient. Normalization is
still available when a text corpus has expensive startup or you need a portable prepared artifact.
...
Use the
[retrieval data preparation tools](https://github.com/NVIDIA-NeMo/Automodel/blob/main/tools/retrieval/README.md) to
create a bundle locally or with the source-parallel Slurm launcher. The same guide explains source selection, per-source
sample caps, append and resume workflows, and the shared-cache alternative when you need to keep the raw dataset config
unchanged.
