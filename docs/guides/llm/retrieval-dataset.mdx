...
Raw retrieval data can use one of three formats: local corpus ID-based JSON, an AutoModel-format Hugging Face dataset
referenced by an `hf://` URI, or inline JSONL. Configure any of these sources with
`nemo_automodel.components.datasets.llm.retrieval_dataset.RetrievalDatasetConfig`. The config chooses the loader from
the URI or file extension. Use the `.jsonl` extension for inline records; a `.json` file is interpreted as corpus-backed
data.
...
| `hf://` AutoModel schema | `question` | `pos_doc`, a companion Hugging Face corpus split, and `neg_doc` before training with `n_passages > 1` | Datasets hosted on Hugging Face that already use the AutoModel retrieval schema |
...
The CPU preparation tool currently accepts corpus ID-based JSON sources. It does not accept `hf://` URIs or inline
JSONL directly. Materialize those sources as corpus ID-based JSON first, or continue using `RetrievalDatasetConfig`.
...
For large datasets on a Slurm cluster, prepare sources in parallel with the CPU array launcher. The
[retrieval data preparation tools](https://github.com/NVIDIA-NeMo/Automodel/blob/main/tools/retrieval/README.md)
document local and Slurm commands, source selection, append and resume behavior, and the cache-warming alternative.

## Training-Time Record Format

Both dataset configs build a Hugging Face `datasets.Dataset` and transform each stored record into the same
...
The cross-encoder recipe consumes the same retrieval records, but sets `model_type: cross_encoder`. Its dataset
...
Each transformed training example uses exactly one positive passage. Both configs use the first item in `pos_doc` by
default. Set `cycle_positive_docs: true` to rotate through multiple positives by epoch. For guidance about expanding
multi-positive data and preventing sibling positives from becoming negatives, refer to
