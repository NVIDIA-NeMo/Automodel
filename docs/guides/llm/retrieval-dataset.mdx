...
## Prepared Normalized Arrow Bundles

For larger text or vision-language retrieval datasets, you can prepare the raw sources once as a portable normalized
Arrow bundle. The bundle keeps query rows as document-ID references and stores each referenced document or image once
in local corpus shards. This avoids rebuilding Hugging Face dataset and corpus state in each GPU training job.
