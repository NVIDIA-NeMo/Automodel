...
Use one of two dataset configs:

- `nemo_automodel.components.datasets.llm.retrieval_dataset.RetrievalDatasetConfig` loads source data directly.
- `nemo_automodel.components.datasets.llm.retrieval_dataset_normalized.NormalizedRetrievalDatasetConfig` loads a
  normalized Arrow bundle prepared in advance.

Both configs build a Hugging Face `datasets.Dataset` and transform each stored record into the same training-time
schema:
...
Raw retrieval data can be corpus-backed or inline. Configure these sources with
`nemo_automodel.components.datasets.llm.retrieval_dataset.RetrievalDatasetConfig`. It selects local corpus-backed JSON,
corpus-backed `hf://`, or inline JSONL loading from the source URI and file extension. Name inline files with the
`.jsonl` extension. An inline `.json` file is treated as corpus-backed data and is not detected automatically.
