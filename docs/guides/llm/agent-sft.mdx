...
The upstream generator currently defines repository names in `src/constants.ts` and constructs clone URLs in `src/pull.ts`. Update both locations in the fork so each matrix entry stores a complete owner and repository pair. Keep the generated Pi session files unchanged. Hugging Face converts those trace files into the OpenAI-format `messages` rows consumed by `make_agent_chat_dataset`.
...
Start the OpenAI-compatible llama.cpp server used by the user model, then generate a small validation batch before scaling up:
...
Point the SynthTraces recipe at that dataset by overriding `dataset.dataset_name` and `validation_dataset.dataset_name`. The dataset must expose parsed OpenAI-format rows through its normal dataset revision or through `refs/convert/parquet`.
...
- **Full-parameter SFT**: The example below shards across 8 GPUs with FSDP2. Qwen2.5-3B can also train unsharded on a single 80 GB GPU because FSDP2 applies only across multiple GPUs.
- **PEFT with LoRA**: You can train on a single GPU. See [Use PEFT with LoRA](#use-peft-with-lora).
- **Dataset size**: The `glaive_toolcall_en` dataset contains a few thousand traces, so a full pass is fast.
...
- `mask_reasoning_content`: Renders assistant `reasoning_content` (thinking) into the prompt but excludes it from the loss.
...
- **`tool_call/accuracy`** (and related `tool_call/*` keys) report generation-based tool-call correctness at each validation step. This metric distinguishes whether the model learned the format or selected the correct tools.

<Note>
Under **FSDP2** (the default strategy here), the in-loop generation evaluation is skipped because generation with sharded weights is expensive. Set `tool_call_eval.run_on_fsdp2: true` to enable it. If you keep it disabled, evaluate tool-call accuracy from a saved checkpoint as described in [Evaluate the Fine-Tuned Model](#evaluate-the-fine-tuned-model).

</Note>

Replace the placeholders in the following illustrative training log with values from your run:
