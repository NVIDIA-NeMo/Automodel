...
- `mask_generation_prompt`: Excludes from the loss the tokens of each assistant turn that the chat template's generation prompt supplies at inference: the role header and any template-inserted empty reasoning block (for example the `<think></think>` that Nemotron and Qwen3 templates emit for turns without `reasoning_content`). The model never generates those tokens, so supervising them only reinforces template boilerplate. Detected by rendering the generation prompt, so it works for any template. Defaults to `false`.
