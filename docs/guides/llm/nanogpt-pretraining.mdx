...
1. The installed `automodel` CLI (`nemo_automodel.cli.app`) is the entry point. It reads the recipe selected by the YAML configuration and wires up the training components. Any configuration option can be overridden using CLI arguments (e.g., `--model.n_layer 6`).
...
Below is the minimal GPT‑2 [implementation](https://github.com/NVIDIA-NeMo/Automodel/blob/main/nemo_automodel/_transformers/models/gpt2.py) used for this NanoGPT‑style pretraining flow.
...
```yaml
model:
  _target_: /abs/path/to/repo/nemo_automodel/_transformers/models/gpt2.py:build_gpt2_model
  vocab_size: 50258
  n_positions: 2048
  n_embd: 768
  n_layer: 12
...
- Any nested mapping that includes `_target_` (e.g., `config:`) is instantiated first and its result is passed upward. This supports both Hugging Face `from_config` callables and direct architecture constructors.
...
**About `_target_` configuration**: The `_target_` field specifies import paths to classes and functions within the nemo_automodel package (or any Python module). For example, `nemo_automodel._transformers.models.gpt2.build_gpt2_model` imports and calls the GPT-2 model builder function. You can also specify paths to your own Python files (e.g., `my_custom_models.MyTransformer`) to use custom `nn.Module` implementations, allowing full flexibility in model architecture while leveraging the training infrastructure.
...
```bash
# Single GPU
uv run automodel examples/llm_pretrain/nanogpt_pretrain.yaml

# Multi-GPU (8 GPUs)
uv run automodel examples/llm_pretrain/nanogpt_pretrain.yaml --nproc-per-node 8
```
<Tip>
