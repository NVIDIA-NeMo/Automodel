...
In this guide, we will use an interactive environment to install NeMo AutoModel from Git. You can also install NeMo AutoModel from PyPI or use our bi-monthly Docker container (see the [Installation Guide](../installation.mdx)).
...
:::{tip}
To run this guide on a single GPU, use the single-GPU command in the **Launch Training** section below and scale down the YAML (for example, reduce `step_scheduler.global_batch_size` / `local_batch_size`, and shrink the model using `model.n_layer` / `model.n_embd` / `model.n_head`). For more launch patterns, see [Run on Your Local Workstation](../../launcher/local-workstation.mdx).
:::
...
For larger datasets, please see [pretraining.mdx](pretraining.mdx) which supports sharded preprocessing without these constraints.
:::
...
:::{tip}
To train on more than 2B tokens from FineWeb, see [pretraining.mdx](pretraining.mdx) which uses Megatron Core's sharded dataset format without file size constraints.
:::
