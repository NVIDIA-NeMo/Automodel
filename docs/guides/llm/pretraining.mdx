...
match the blend order. JSON blend files and implicit unweighted blends are not
supported. Sequence packing is not supported with `domain_mixture` because
...
The logged `domain_mixture/<name>_label_tokens` counts exclude the configured
loss `ignore_index`. Datasets that mark padding through a separate `loss_mask`
