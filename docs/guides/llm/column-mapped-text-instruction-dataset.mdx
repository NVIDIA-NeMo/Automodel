...
For **streaming** (including **Delta Lake / Databricks**), use [`ColumnMappedTextInstructionIterableDataset`](column-mapped-text-instruction-iterable-dataset.mdx). The iterable variant always streams by design to avoid accidentally materializing entire datasets to disk/memory.
...
:::{note}
`ColumnMappedTextInstructionDataset` does not support streaming or Delta Lake / Databricks sources. For those, use [`ColumnMappedTextInstructionIterableDataset`](column-mapped-text-instruction-iterable-dataset.mdx).
:::

:::{note}
Delta Lake / Databricks (including `delta_sql_query` and authentication) is supported only by `ColumnMappedTextInstructionIterableDataset`. See [`column-mapped-text-instruction-iterable-dataset.mdx`](column-mapped-text-instruction-iterable-dataset.mdx) for details.
:::
...
live in your sbatch script. See the [cluster guide](../../launcher/slurm.mdx) for
