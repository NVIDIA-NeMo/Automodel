...
the pretrained-only Base BF16 checkpoint
`nvidia/NVIDIA-Nemotron-3.5-Super-VL-120B-A12B-Base-BF16`. The complete
...
```bash
export MODEL=nvidia/NVIDIA-Nemotron-3.5-Super-VL-120B-A12B-Base-BF16
...
export MODEL_REVISION="${MODEL_SNAPSHOT##*/}"
```

The snapshot directory records the resolved commit. Reuse this `MODEL_REVISION` for
subsequent runs to pin the model and tokenizer to the same version.
