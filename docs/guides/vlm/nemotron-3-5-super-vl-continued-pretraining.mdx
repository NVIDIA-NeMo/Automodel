...
Open the online W&B run and select **Charts**. In the `loss` panel, use **Step**
...
against the corresponding Slurm log entry within its printed precision. The screenshot
below shows the W&B `loss` panel with smoothing set to `0`.

![W&B raw training loss for Super 3.5 VL FineWeb continued pretraining](./nemotron-3-5-super-vl-fineweb-cpt-loss.png)
...
updates. Each point is the raw training loss for that step. Full-precision history is
