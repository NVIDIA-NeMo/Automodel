...
[NeMo Automodel](https://github.com/NVIDIA-NeMo/Automodel). Full SFT on 8 nodes or LoRA on a single node,
...
| **Step 3** | Launch fine-tuning (8 nodes for SFT, 1 node for LoRA) |
...
or on any 8-GPU machine: `automodel examples/vlm_finetune/nemotron_3_5_super_vl/nemotron_3_5_super_vl_cord_v2_peft.yaml --nproc-per-node 8`.
...
| GPU memory | ~31.8 GiB / GPU (64 GPUs) | ~39 GiB / GPU (8 GPUs) |
| Training time | ~26 min on 8x8 H100 (~32 min wall clock) | ~13 min on 1x8 H100 |
| Best val loss | 0.061 (step 99) | 0.039 (step 199) |
| Final train loss | 0.0046 (step 399; mean of last 10 steps 0.029) | 0.0067 (step 399; mean of last 10 steps 0.014) |
| Checkpoint size | ~227 GB (consolidated bf16) | ~355 MB (adapter only) |
| Exact matches (20 val) | 8/20 at step 99 (`LOWEST_VAL`, mean similarity 0.96); 6/20 at step 399 (0.89) | 8/20 at step 199 (`LOWEST_VAL`, mean similarity 0.95) |
