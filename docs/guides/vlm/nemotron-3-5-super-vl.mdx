...
- **SFT memory**: ~65 GiB per GPU (fp32 master weights, bf16 Adam moments, activation checkpointing). The frozen
  RADIO tower is replicated on every rank (`frozen_sharding: replicate`) and checkpoint state dicts are moved to
  CPU before they are written (`checkpoint.cpu_offload: true`), so the validation-time saves need no GPU headroom.
...
`cp_size=4` on 32 GPUs maps the 8-sample global batch to one sample per rank, the optimizer keeps **fp32 master weights** with
...
or with the launcher: `automodel examples/vlm_finetune/nemotron_3_5_super_vl/nemotron_3_5_super_vl_cord_v2.yaml --nproc-per-node 8 --nnodes 4`.
...
Validation loss is lowest at step 299 (`LOWEST_VAL`); that checkpoint is stored sharded, so run its
`model/consolidate.sh` and point `eval_cord_v2.py` at the export to score it the same way (the final step is the
only one exported inline).
...
| 3 | `S-Ovaltine 50%` | Missing `<s_vatyn>`; items nested in an extra `<s_sub>` block | 0.72 |
| 4 | `70%, Less Ice, M-Caramel Black Tea` | Same fields, one value differs | 0.99 |
| 5 | `Sedang, BBQ Chicken` | `<s_menutype_cnt>` where the label has `<s_menuqty_cnt>` | 0.61 |
| 6 | `LE MINERAL` | Extra menu item (`<s_nm>`, `<s_price>`, `<s_unitprice>`) | 0.61 |
| 7 | `POTATO SAUSAGE BREAD, OREO GREEN TEA SPREAD, ...` | Exact match | 1.00 |
| 8 | `Choco Devil` | Missing `<s_discountprice>`; extra menu item | 0.86 |
...
| Exact matches (20 val) | 5/20 at step 399 (`LATEST`, mean similarity 0.92) | 8/20 at step 199 (`LOWEST_VAL`, mean similarity 0.95) |
