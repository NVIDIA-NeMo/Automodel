...
position: 15
---
**A step-by-step guide for fine-tuning Nemotron 3.5 Super VL (121B hybrid Mamba+Attention MoE VLM)
to extract structured receipt data from scanned images using
[NeMo Automodel](https://github.com/NVIDIA-NeMo/Automodel). Full SFT on 8 nodes or LoRA on 2 nodes,
...
As in the [NemotronOmni guide](/recipes-e2e-examples/nemotron-omni), we fine-tune on **CORD-v2**
...
| **Step 3** | Launch fine-tuning (8 nodes for SFT, 2 nodes for LoRA) |
...
- **SFT memory**: ~31.8 GiB per GPU (fp32 master weights, bf16 Adam moments, activation checkpointing),
  leaving headroom for the validation-time checkpoint saves.
- **Training time**: ~26 min on 8x8 H100 for 400 steps (4 epochs of the 800 training samples), including four
  ...
  with model loading
- **LoRA PEFT**: 2 nodes x 8 H100 (`cp_size 2` / `ep_size 16`, ~22 GiB per GPU); 4 nodes with `cp_size 4` /
  `ep_size 32` run at ~15 GiB per GPU. The frozen base needs no fp32 master copy or optimizer state.
...
bf16 Adam moments, and the learning rate is lowered to 5e-5 for full-parameter SFT of a 121B model.
...
distributed:
  strategy: fsdp2
  cp_size: 2            # dp 8 on 16 GPUs -> one sample per rank per step
  ep_size: 16
...
optimizer:
  ...
  weight_decay: 0.01
  betas: [0.9, 0.95]
  master_weights: true              # fp32 master copy of the bf16 LoRA weights, as in the SFT recipe
  store_param_remainders: true
  exp_avg_dtype: torch.bfloat16
  exp_avg_sq_dtype: torch.bfloat16
```
...
**2 nodes x 8 H100** (~22 GiB per GPU; 4 nodes with `cp_size 4` / `ep_size 32` run at ~15 GiB per GPU).
...
step    0 | loss 0.5060 | grad_norm  1.98 | lr 2.00e-04 | mem 22.18 GiB
step   10 | loss 0.0973 | grad_norm  0.70 | lr 2.00e-04 | mem 22.27 GiB
step   50 | loss 0.0528 | grad_norm  0.30 | lr 2.00e-04 | mem 22.27 GiB
step  100 | loss 0.0277 | grad_norm  0.27 | lr 2.00e-04 | mem 22.27 GiB
step  200 | loss 0.0065 | grad_norm  0.11 | lr 2.00e-04 | mem 22.27 GiB
step  300 | loss 0.0205 | grad_norm  0.44 | lr 2.00e-04 | mem 22.28 GiB
step  399 | loss 0.0053 | grad_norm  0.13 | lr 2.00e-04 | mem 22.26 GiB

Validation:
  step  99 | val_loss 0.0449
  step 199 | val_loss 0.0402  <-- LOWEST_VAL
  step 299 | val_loss 0.0438
  step 399 | val_loss 0.0403
```

400 steps take ~18 min on 2x8 H100. Each checkpoint holds the adapter only (~355 MB), so
...
121B model to start memorizing it.
...
HF module tree. `eval_cord_v2.py --adapter` folds the adapters into the base weights
(`W += (B @ A) * alpha / r`) before decoding:

```bash
python examples/vlm_finetune/nemotron_3_5_super_vl/eval_cord_v2.py \
    --checkpoint nvidia/NVIDIA-Nemotron-3.5-Super-midtrain-67B-vision-pretrained \
    --adapter vlm_checkpoints/nemotron_3_5_super_vl_cord_v2_peft/LOWEST_VAL/model \
    --split validation --num-samples 20
```
...
| LoRA PEFT, step 199 (`LOWEST_VAL`, val loss 0.040) | **10/20** | **0.958** (median 0.996; 16/20 above 0.9) | 20/20 |
| LoRA PEFT, step 399 (`LATEST`, lr=2e-4, 400 steps) | 8/20 | 0.965 (median 0.996; 17/20 above 0.9) | 20/20 |
...
| GPU memory | ~31.8 GiB / GPU (64 GPUs) | ~22 GiB / GPU (16 GPUs) |
| Training time | ~26 min on 8x8 H100 (~32 min wall clock) | ~18 min on 2x8 H100 |
| Best val loss | 0.061 (step 99) | 0.040 (step 199; 0.040 again at step 399) |
| Final train loss | 0.0046 (step 399; mean of last 10 steps 0.029) | 0.0053 (step 399; mean of last 10 steps 0.014) |
| Checkpoint size | ~227 GB (consolidated bf16) | ~355 MB (adapter only) |
| Exact matches (20 val) | 8/20 at step 99 (`LOWEST_VAL`, mean similarity 0.96); 6/20 at step 399 (0.89) | 10/20 at step 199 (`LOWEST_VAL`, mean similarity 0.96); 8/20 at step 399 (0.97) |
