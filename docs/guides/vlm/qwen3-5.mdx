...
<Info>
Long-context Qwen3.5 runs can require setup-time prewarming even when later training steps fit in memory, because first-use CUDA and communicator allocations overlap with the first backward peak. The short-context MedPix recipe does not enable prewarming by default. If a longer hybrid-attention MoE run fails only on its first step, see [Prewarm One-Time CUDA Initialization](/get-started/configuration#prewarm-one-time-cuda-initialization) and enable only the warmup matching the failure signature. This does not by itself establish support for a particular context length.
