...
Context-parallel vision sharding distributes independent image and video frames across the
CP group. Each rank processes only its assigned frames, and the resulting visual embeddings
are gathered in their original order before the existing language-sequence shard. This
reduces duplicated vision-tower computation while preserving the model's input ordering and
gradients.

The built-in integration supports the following model families:
...
## Configure Vision Sharding

Enable context parallelism and vision sharding under `distributed`:
...
Vision sharding is disabled by default. A CP size greater than 1 and a supported model are
required for the sharded path.

The following example recipes are available:

- [Dense Qwen3-VL 8B with CP2 and vision sharding](https://github.com/NVIDIA-NeMo/Automodel/blob/main/examples/vlm_finetune/qwen3/qwen3_vl_8b_cp2_vision_shard.yaml)
- [Dense Qwen3.5 4B with CP2 and vision sharding](https://github.com/NVIDIA-NeMo/Automodel/blob/main/examples/vlm_finetune/qwen3_5/qwen3_5_4b_cp2_vision_shard.yaml)

For example, run the Qwen3-VL recipe on eight GPUs:

```bash
uv run automodel --nproc-per-node=8 \
  examples/vlm_finetune/qwen3/qwen3_vl_8b_cp2_vision_shard.yaml
```
...
|---|---|---|
| `enabled` | `false` | Enables vision sharding across the CP group. |
| `min_tokens` | `2048` | Uses the replicated path when the batch has fewer visual tokens than this threshold. Set it to `0` to always exercise vision sharding. |
...
sharding uses this property to:
...
uv run automodel --nproc-per-node=8 \
  examples/vlm_finetune/qwen3/qwen3_vl_8b_cp2_vision_shard.yaml \
  --distributed.cp_vision_sharding.enabled false
```
...
- This setting is a frame-level work-sharing policy over the existing CP group, not
  a topology declaration for the encoder. It does not assign separate TP, CP, or PP
  meshes to vision, audio, and language submodules.
...
- Activation checkpointing already reduces retained vision activations, so vision sharding
...
- Additional VLM families require model-specific integration before they can use this
  policy.
- CP vision sharding has no effect when `cp_size` is `1`.
