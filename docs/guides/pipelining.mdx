...
ap = AutoPipeline(
    ...
    pp_schedule="1f1b",             # Pipeline schedule ("1f1b", "LoopedBFS", etc.)
...
# Use pipeline_model only with an HF-compatible model.
schedule, model_parts, has_first, has_last, stages = pipeline_model(
    model=hf_compatible_model,
    world_mesh=world_mesh,
    moe_mesh=None,
    pp_axis_name="pp",
    dp_axis_names=("dp",),
...
pipeline_stages = info.stages  # List of local PipelineStage objects used by the schedule
stage_modules = ap.list_stage_modules()  # Module names within each local model part
```
...
### Gradient Management
```python
from nemo_automodel.components.distributed.pipelining.functional import scale_grads_by_divisor
