...
The table below tracks when model support and key features were added across NeMo AutoModel releases. For the full list of tested architectures and example configs, see the [LLM](/model-coverage/large-language-models/overview) and [VLM](/model-coverage/vision-language-models/overview) pages.
