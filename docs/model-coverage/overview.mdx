...
This is a practical starting set rather than an exhaustive compatibility list. NeMo AutoModel can also work with additional models supported by the installed version of Hugging Face `transformers`. Models without a checked-in recipe might require some configuration for a particular training setup.
