...
| Training | 400 steps (global batch 8, 4 epochs of the 800 training receipts) in 18 min on 2 nodes; train loss 0.51 → 0.01 |
| Validation loss | 0.045 / 0.040 / 0.044 / 0.040 at steps 99 / 199 / 299 / 399 |
| Field extraction (first 20 validation receipts, greedy, adapter merged into the base model) | step 199 (`LOWEST_VAL`): 10/20 exact match, mean similarity 0.958, 20/20 structured outputs; step 399: 8/20, 0.965, 20/20 |
...
**2. Run the vision recipe** from inside the repo (8 nodes × 8 H100):

```bash
uv run automodel examples/vlm_finetune/nemotron_3_5_super_vl/nemotron_3_5_super_vl_medpix_100steps.yaml --nnodes 8 --nproc-per-node 8
```
