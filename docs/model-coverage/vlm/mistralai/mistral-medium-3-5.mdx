...
description: "Use Devstral-2-123B-Instruct-2512 with NeMo AutoModel for vision-language fine-tuning, with documented checkpoints, runnable recipes, setup, and model reference details."
...
`weight_scale_inv`) so the full model fits inside an H200 node or 2 x
...
<Note>
This recipe was validated on **8 nodes x 8 GPUs (64 H100s)** with
...
**1 x H200** or **2 x H100** nodes thanks to the dense + FP8 layout.

</Note>

Submit the recipe through Slurm (see the
...
| Goal | Start Here |
| --- | --- |
| Supervised fine-tuning (SFT) - Mistral Medium 3.5 128B on MedPix, 8 nodes x 8 GPUs (TP=8 PP=8) | Use [mistral3p5_128b_medpix.yaml](https://github.com/NVIDIA-NeMo/Automodel/blob/main/examples/vlm_finetune/mistral3p5/mistral3p5_128b_medpix.yaml). Dataset: MedPix-VQA. |
...
| License | Modified MIT (open weights, up to $20M annual revenue threshold) |
...
- **Compactness.** Dense 128B fits in fewer GPUs than the comparable
  MoE class - a single H200 node or 2 x H100 nodes for inference.
