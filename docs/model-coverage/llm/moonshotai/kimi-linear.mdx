...
- `KimiLinearForCausalLM` (`model_type: kimi_linear`)
...
| Context parallelism (CP) | Contiguous per-rank sequence shards; KDA passes its recurrent state rank to rank, MLA gathers the compressed KV latent |
