...
position: 2
---
NeMo AutoModel provides GPU-accelerated, `transformers`-compatible training for supported LLMs and VLMs. It combines Hugging Face model loading with NVIDIA's distributed training stack while keeping recipes and configuration visible.

## Why NeMo AutoModel?

- **Hugging Face native**: Train supported Hub models without converting checkpoints into a NeMo-only format.
- **High performance**: Custom CUDA kernels such as TransformerEngine, DeepEP, and FlexAttn are available for supported architectures. See the benchmark page for methodology and caveats.
- **Scales through configuration**: Recipes can move from local launch to multi-node launch when the model, dataset, hardware, and distributed strategy support it.
...
Copy `slurm.sub`, edit the `CONFIG`, `#SBATCH`, container, mount, and secret settings for your cluster, then submit with `sbatch`. See the [Cluster guide](/job-launchers/slurm-cluster).
</Card>
