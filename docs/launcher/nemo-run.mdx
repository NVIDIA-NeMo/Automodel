...
description: "Submit NeMo AutoModel recipes through NeMo-Run executors for Slurm, Kubernetes, Docker, or local backends."
position: 4
---
...
def my_slurm_cluster():
    ...
    executor.container_image = "nvcr.io/nvidia/nemo-automodel:26.06.00"
...
The CLI detects the `nemo_run:` key, strips it from the training config, loads the named executor from `$NEMORUN_HOME/executors.py`, and submits the job -- all in one command. A healthy submission prints the resolved executor name and a NeMo-Run experiment/job location. On Slurm, confirm placement with `squeue -u $USER` and inspect the NeMo-Run job directory for the generated script and launcher logs before debugging the training recipe itself.
