---
title: "Run NeMo AutoModel on a Slurm Cluster"
description: "Submit NeMo AutoModel training jobs on single-node or multi-node Slurm clusters using the reference sbatch script."
position: 3
---
...
Expected result: Slurm prints a submitted batch job ID. Use `squeue -j <job_id>`
to confirm the job is queued or running, and check the `slurm_jobs/` output and
error files configured in the script for training logs.
...
```bash
squeue -j <job_id>
tail -f slurm_jobs/<job_name>_<job_id>.out
```

The log should show `torchrun -m nemo_automodel.cli.app` starting the configured recipe. If the job exits before training starts, check the `.err` file for account/partition errors, container image access, missing mounts, Hugging Face cache or token issues, and NCCL rendezvous failures.
