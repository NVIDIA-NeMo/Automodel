...
description: "Launch NeMo AutoModel recipes on cloud VMs or Kubernetes through SkyPilot, including provider setup, YAML fields, and job cleanup."
position: 5
---
...
SkyPilot is an open-source framework that abstracts cloud infrastructure so the same AutoModel recipe can run on the provider or Kubernetes cluster you configure. It can also request spot instances when your workload can tolerate preemption.
