---
title: "SkyPilot Kubernetes"
description: "Run NeMo AutoModel on a Kubernetes GPU cluster with SkyPilot, from cluster checks through single-node and multi-node launches."
position: 6
---
...
A healthy setup shows GPUs in this output. If the table is empty, use the [`sky show-gpus --infra k8s` troubleshooting section](#sky-show-gpus-infra-k8s-shows-no-gpus) to finish Kubernetes and SkyPilot GPU registration before launching training.
...
SkyPilot schedules the GPUs Kubernetes exposes. Use your Kubernetes GPU device plugin or operator checks to confirm the nodes are healthy, then rerun `sky show-gpus --infra k8s`.

### The job starts, but model download fails

For gated models, export `HF_TOKEN` in the shell that runs `automodel`. The SkyPilot launcher forwards it to the remote job.
