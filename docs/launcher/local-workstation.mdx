...
description: "Run NeMo AutoModel recipes on a single machine with the automodel CLI, uv, or torchrun, including single-GPU and multi-GPU launch patterns."
position: 2
---
...
The commands below use checked-in recipes, so run them from a source checkout:
...
# Llama is gated on Hugging Face. Accept model terms, then authenticate once.
...
Expected result: the run prints the selected recipe and model, loads the dataset, and logs `train_loss` for a few steps. The smoke-test overrides shorten the run and validation set; remove them for normal training. For gated Hugging Face models, accept the model terms and provide credentials with `HF_TOKEN` or `huggingface-cli login`.

## Run with AutoModel CLI (Single Node)

The AutoModel CLI is the preferred method for most users. It launches the same recipe YAML locally, on a single multi-GPU node, or through the cluster launcher patterns in [Run on a Cluster](/job-launchers/slurm-cluster).
