...
|---|---|---|---|
| **Local Workstation** | Getting started, debugging, single-node training | 1-8 on one machine | [Local Workstation](./local-workstation.mdx) |
| **NeMo-Run** | Managed execution on Slurm, Kubernetes, Docker, local | 1+ | [NeMo-Run](./nemo-run.mdx) |
| **SkyPilot** | Cloud training or Kubernetes clusters | Any | [SkyPilot](./skypilot.mdx) |
| **Slurm** | Multi-node batch jobs on HPC clusters | 8+ across nodes | [Slurm](./slurm.mdx) |
...
See the [Local Workstation](./local-workstation.mdx) guide.
...
See the [NeMo-Run](./nemo-run.mdx) guide.
...
See the [SkyPilot](./skypilot.mdx) guide.
...
See the [SkyPilot + Kubernetes tutorial](./skypilot-kubernetes.mdx).
