...
- [**Day-0 support**](model-coverage/overview.mdx) for LLMs and VLMs available on the Hugging Face Hub.
...
{/* For an in-depth explanation of the LLM recipe please also see the [LLM recipe deep-dive guide](docs/llm_recipe_deep_dive.mdx). */}
...
- **Your local workstation**: See [`docs/launcher/local-workstation.mdx`](launcher/local-workstation.mdx).
- **A cluster**: See [`docs/launcher/slurm.mdx`](launcher/slurm.mdx).
