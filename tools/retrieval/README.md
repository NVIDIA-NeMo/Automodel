...
On Slurm CPU nodes, use the array wrapper for full datasets. It prepares each original source independently and then
runs a small dependent finalizer job to write the top-level metadata:
...
For the Nemotron VL 1B image-retrieval mix used during migration, `TIME=04:00:00` is a reasonable default.

The non-array `submit_prepare_normalized_vl_retrieval_data_cpu.sh` helper is useful for smoke tests and small source
lists, but it serializes sources and can take many hours on the full VL retrieval mix.
...
corpus directories from the same output directory. For the array wrapper, set the same `NUM_SOURCES`/`ARRAY_SPEC`; each
source task resumes its own `sources/source-*` directory and the finalizer refreshes the top-level metadata. Resume is
for continuing an interrupted prep run; append mode is for adding new source entries to an already prepared bundle.
...
| Normalized Arrow | Train refs plus local corpus Arrow shards, with documents deduplicated within each prepared source | `176G` observed for the earlier globally deduplicated bundle; source-preserving bundles can be larger if sources share documents | Recommended full-dataset portable format |
