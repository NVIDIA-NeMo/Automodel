...
```bash
# Local interactive launch; the YAML recipe selects LLM, VLM, diffusion, or retrieval behavior.
...
torchrun --nproc-per-node 8 -m nemo_automodel.cli.app examples/retrieval/bi_encoder/llama3_2_1b.yaml
```

Override any config value from the CLI:

```bash
automodel config.yaml --model.name_or_path meta-llama/Llama-3.2-1B
```
