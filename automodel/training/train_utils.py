...
def training_log(
    ...
    batch_size = train_config.micro_batch_size * config.data_parallel_size * num_microbatches
    ...
    if logger_config.log_timers_to_tensorboard and (train_state.step % logger_config.tensorboard_log_interval == 0):
        timers.write(timers_to_log, tb_logger, train_state.step, normalizer=total_iterations, reset=False)
        timers.write_to_wandb(timers_to_log, wandb_logger, train_state.step, normalizer=total_iterations, reset=True)
