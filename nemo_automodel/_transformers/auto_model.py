...
from nemo_automodel._transformers.models.common.tie_word_embeddings import reject_tie_word_embeddings_flip
...
_RETRIEVAL_AUTO_MODEL_ATTRS = frozenset({"NeMoAutoModelBiEncoder", "NeMoAutoModelCrossEncoder"})


def __getattr__(name: str):
    """Lazily resolve retrieval auto-models from their canonical package."""
    if name in _RETRIEVAL_AUTO_MODEL_ATTRS:
        from nemo_automodel.retrieval import auto_model as _retrieval_auto_model

        attr = getattr(_retrieval_auto_model, name)
        globals()[name] = attr
        return attr
...
def __dir__():
    """Include compatibility auto-model names in module introspection."""
    return sorted(set(globals()) | _RETRIEVAL_AUTO_MODEL_ATTRS)
