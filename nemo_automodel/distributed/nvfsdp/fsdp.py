...
class FSDP(torch.nn.Module):
    ...
    def __init__(
        ...
        if isinstance(self.device_mesh, DeviceMesh):
            ...
            ), "Data parallel process group (dp_cp) is required when using custom FSDP."
    ...
    def _replace_param_with_distributed_if_needed(self):
        if (
            self.ddp_config.data_parallel_sharding_strategy
            not in ["optim", "optim_grads", "optim_grads_params"]
            or self.is_param_fsdp_distributed
        ):
            return
    ...
    def _replace_param_with_raw_if_needed(self):
        if (
            self.ddp_config.data_parallel_sharding_strategy
            not in ["optim", "optim_grads", "optim_grads_params"]
            or not self.is_param_fsdp_distributed
        ):
            return
        self.is_param_fsdp_distributed = False
