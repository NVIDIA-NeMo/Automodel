...
class DataParallelBuffer:
    ...
    def fetch_bucket(self, dtype: Optional[torch.dtype] = None) -> Bucket:
        """
    ...
    for group in bucket_groups:
        ...
        while len(params) > 0:
            ...
            for param in params:
                ...
                if param_shape[1:].numel() == chunk_size_factor:
...
class ParamAndGradBuffer:
    ...
    def _init_distributed_params(self):
        ...
        self.dist_model_weight = dist_model_weight
    ...
    def _init_optimizer_named_parameters(self) -> List[Tuple[str, torch.nn.Parameter]]:
        ...
        for pg in self.parameter_groups:
            for item_id, orig_param in enumerate(pg.params):
                ...
                elif param_name in self.dist_model_weight:
                    param_data = self.dist_model_weight[param_name]
                else:
                    raise ValueError(
                        f"Parameter {param_name} not found in dist model weight or "
                        f"dist main weight."
                    )
    ...
    def update_main_grads(self):
        ...
        for _, param in self.optimizer_named_parameters:
            ...
            optimizer_grad = group.main_grad_buffer.get_item(item_id)
            ...
            if gbuf.is_data_distributed:
                grad = _fsdp_tensor(
                    ...
                    dp_mesh=DeviceMesh.from_group(
                        group.main_grad_buffer.data_parallel_group, device_type="cuda"
                    ),
                )
...
def _get_fsdp_tensor_spec(param, dp_mesh):
    if isinstance(param, DTensor):
        tp_spec = cast(DTensor, param)._spec
    ...
    return dp_mesh, [Shard(0)]


def _fsdp_tensor(data, param, dp_mesh, run_check=False):
    """
    ...
    Args:
        ...
        dp_mesh (torch.distributed._tensor.DeviceMesh): The device mesh for data parallelism.
    ...
    """
    fsdp_mesh, placements = _get_fsdp_tensor_spec(param, dp_mesh)
    param_shape = to_local_if_dtensor(param).shape
