...
def make_cp_batch_and_ctx(
    ...
    force_model_cp_path = os.environ.get("NEMO_AUTOMODEL_DEBUG_FORCE_GEMMA4_CP_PATH", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
    }
    if (_get_mesh_size(cp_mesh) > 1 or force_model_cp_path) and cp_make_batch_fn is not None:
