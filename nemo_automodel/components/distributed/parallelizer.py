...
from nemo_automodel.components.distributed.optimized_tp_plans import PARALLELIZE_FUNCTIONS, PARALLELIZE_FUNCTIONS_FOR_MODEL_ARCH
...
def validate_tp_mesh_for_decilm(model, tp_size):
...
def validate_tp_mesh(model, tp_mesh):
    ...
    elif model_arch == "DeciLMForCausalLM":
        validate_tp_mesh_for_decilm(model, tp_mesh.size())
...
def _get_parallel_plan(
    ...
    elif model_arch in PARALLELIZE_FUNCTIONS_FOR_MODEL_ARCH:
        try:
            func = PARALLELIZE_FUNCTIONS_FOR_MODEL_ARCH[model_arch]
            model_parallel_plan = func(model, sequence_parallel)
            print("Using optimized parallel plan.")
        except Exception as e:
            print(f"Optimized parallel plan is not available: {e}. Falling back to the HF tp plan.")
