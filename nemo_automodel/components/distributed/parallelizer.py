...
class DefaultParallelizationStrategy(ParallelizationStrategy):
    ...
    def parallelize(
        ...
        # TE ring-attention setup needs a CUDA stream and a real GPU process group, so it is
        # only meaningful on CUDA devices. Skip on CPU (e.g. CPU-only unit tests) to avoid
        # spurious torch.cuda.Stream() / get_process_group_ranks failures.
        if (
            cp_mesh is not None
            and cp_mesh.size() > 1
            and getattr(device_mesh, "device_type", None) == "cuda"
