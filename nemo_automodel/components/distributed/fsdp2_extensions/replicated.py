...
class _ReplicatedGradSync:
    ...
    Attributes:
        parameters: Replicated parameter tensors of arbitrary shape. DTensors may
            retain TP placements but must be replicated over ``mesh``.
    ...
    def synchronize(self) -> None:
        if not self.parameters or self.mesh.size() == 1:
            return

        local_parameters = tuple(_local_tensor(parameter) for parameter in self.parameters)
        device = local_parameters[0].device
        # One FP32 payload carries a rank-symmetric error flag, one local-use bit
        # per parameter, and every gradient. This preserves one collective per
        # mesh dimension even when a parameter is unused on only some ranks.
        header_size = 1 + len(self.parameters)
        ...
        offset = header_size
        for index, (parameter, local_parameter) in enumerate(zip(self.parameters, local_parameters)):
            if local_parameter.device != device:
                flat_grad[0] = 1
            grad = parameter.grad
            if grad is not None:
                flat_grad[1 + index] = 1
        ...
        offset = header_size
        for parameter, local_parameter, used in zip(self.parameters, local_parameters, globally_used):
...
def make_fully_shard_with_replicated_parameter_grad_sync(
    ...
    *,
    ...
    grad_sync = _ReplicatedGradSync(trainable, mesh)
    installed = False
