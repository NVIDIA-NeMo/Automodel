...
def paged_stash_copy_kernel(
    ...
    while token_index < num_tokens:
        ...
        for block_index in range(tl.cdiv(HIDDEN_SIZE, BLOCK_SIZE)):
            ...
            values = tl.load(source_base + offsets, mask=mask, other=0)
            tl.store(destination_base + offsets, values, mask=mask)
        token_index += num_programs
...
def paged_stash_pop_kernel(
    ...
    while token_index < num_tokens:
        ...
        destination_base = dst_ptr + token_index.to(tl.int64) * HIDDEN_SIZE
        for block_index in range(tl.cdiv(HIDDEN_SIZE, BLOCK_SIZE)):
            offsets = tl.arange(0, BLOCK_SIZE) + block_index * BLOCK_SIZE
            mask = offsets < HIDDEN_SIZE
            values = tl.load(source_base + offsets, mask=mask, other=0)
