...
class HybridEPCombineMetadata:
    ...
    Attributes:
        group: Process group whose ranks own equal contiguous expert ranges.
        source_rows: Integer tensor [outgoing_routes], indexing this rank's
            padded input tokens in destination-rank order.
        source_slots: Integer tensor [padded_local_tokens, maximum_routes],
            indexing returned rows in expert-ascending order, with -1 for unused
            slots. maximum_routes is this rank's maximum selected experts/token.
        expert_rows: Integer tensor [incoming_routes], indexing received routes
            in HybridEP's padded, expert-major output.
        send_splits: Outgoing route counts in process-group rank order.
        receive_splits: Incoming route counts in process-group rank order.
        num_tokens: Padded local token count. All index tensors use the same
            device as the dispatch activations and remain immutable for backward.
    """
...
class _HybridEPCombineInFP32(torch.autograd.Function):
    """Return individual routed outputs and reduce locally in deterministic order."""

    @staticmethod
    def forward(
        ctx: torch.autograd.function.FunctionCtx, expert_output: torch.Tensor, metadata: HybridEPCombineMetadata
    ) -> torch.Tensor:
        """Return individual expert contributions before a local FP32 sum.

        Args:
            ctx: Autograd context retaining route metadata and input layout.
            expert_output: BF16 tensor [padded_expert_rows, hidden], ordered by
                local expert exactly as in the corresponding HybridEP dispatch.
            metadata: Immutable route tensors documented by HybridEPCombineMetadata.

        Returns:
            Independently stored FP32 tensor [padded_local_tokens, hidden].
            Expert contributions are summed in ascending global expert order.
        """
    ...
    def backward(ctx: torch.autograd.function.FunctionCtx, grad_output: torch.Tensor) -> tuple[torch.Tensor, None]:
        """Reverse the sparse exchange with zero gradients for expert padding.

        Args:
            ctx: Saved inverse routes, expert-output shape and dtype.
            grad_output: Tensor [padded_local_tokens, hidden] from the routed sum.

        Returns:
            Independent expert gradient [padded_expert_rows, hidden] in the
            forward input dtype, followed by no gradient for route metadata.
        """
...
def hybridep_combine_in_fp32(expert_output: torch.Tensor, metadata: HybridEPCombineMetadata) -> torch.Tensor:
    """Return expert outputs through the active dispatch's inverse route map.

    Args:
        expert_output: BF16 tensor [padded_expert_rows, hidden], ordered by local
            expert exactly as produced by the corresponding HybridEP dispatch.
        metadata: Immutable tensor layouts documented by HybridEPCombineMetadata,
            on the expert-output device and associated with that same dispatch.

    Returns:
        Independent FP32 tensor [padded_local_tokens, hidden], with zero rows
        where no expert was selected. Backward returns gradients to expert owners.
    """
