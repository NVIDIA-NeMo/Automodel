...
def _record_stream_safe(t: torch.Tensor, stream: Optional[torch.cuda.Stream]) -> None:
    """Register a cross-stream use of ``t``'s storage with the caching allocator.

    When a tensor allocated on one CUDA stream is read or written on another, the
    caching allocator may recycle its block while the other stream is still using it
    (a cross-stream use-after-free that silently corrupts values). ``record_stream``
    tells the allocator to defer reuse until ``stream`` is done. No-op for non-CUDA
    tensors; a DTensor is recorded on its local shard.

    Args:
        t: The tensor whose storage is used on ``stream``.
        stream: The stream that will use ``t``; ``None`` is a no-op.
    """
...
class _SharedExpertStreamFork(torch.autograd.Function):
    """Fork the shared-expert input onto the side stream with autograd-safe fencing.

    A data-identity op that only enforces cross-stream ordering and allocator safety
    so the shared-expert/dispatch overlap is correct in BOTH passes. A raw
    ``wait_stream`` is forward-only and has no backward mirror, which (together with a
    missing ``record_stream``) is the cross-stream race that corrupts gradients.

    Forward (main stream): the side stream waits for the main stream so it observes
    ``x``. Backward (main stream): the main stream waits for the side stream before
    ``x``'s gradient (produced by the shared-expert subgraph on the side stream) is
    accumulated alongside the main-stream branches.
    """
...
class _SharedExpertStreamJoin(torch.autograd.Function):
    """Join the shared-expert output back to the main stream with autograd-safe fencing.

    Forward (main stream): the main stream waits for the side stream so the
    shared-expert output is ready before it is added to the routed output; applied
    AFTER ``experts()`` is launched so the shared-expert compute overlaps the dispatch
    comm. Backward (main stream): the side stream waits for the main stream before the
    shared-expert subgraph (replayed on the side stream) consumes the joined gradient.
    """
...
class MoE(nn.Module):
    ...
    def forward(
        ...
        # Shared-expert output (optionally gated). Run on a side CUDA stream to overlap
        # the shared-expert MLP compute with the routed-expert dispatch/combine comm.
        # The fork/join autograd fences (_SharedExpertStreamFork / _SharedExpertStreamJoin)
        # make the overlap correct in BOTH forward and backward: a raw wait_stream is
        # forward-only, and its missing backward mirror plus the missing record_stream is
        # the cross-stream race that corrupts gradients (grad-norm explosion at high
        # tokens/microbatch). shared_experts stays in the normal autograd graph, so FSDP2's
        # post-backward reduce-scatter hooks and gradient accumulation are unaffected;
        # expert parallelism only touches the routed ``experts`` path on the main stream.
        z = None
        side_stream = None
        if self.shared_experts is not None and x.is_cuda:
            ...
            # No CUDA stream available (e.g. CPU): compute inline.
            z = self.shared_experts(x)
            if self.shared_expert_gate is not None:
                z = torch.nn.functional.sigmoid(self.shared_expert_gate(x)) * z

        # Routed experts on the main stream -- runs concurrently with the shared-expert
        # side stream above; the join below waits for it.
