...
class MoESplitExpertsStateDictMixin:
    ...
    def _validate_expert_availability(
        ...
        # Build prefix pattern with known prefixes + any detected custom prefix
        known_prefixes = {"", "model.", "language_model.", "model.language_model."}
        if key_prefix:
            known_prefixes.add(key_prefix)

        prefix_alternatives = "|".join(re.escape(p) for p in sorted(known_prefixes, key=len, reverse=True))
        prefix_pattern = f"(?P<prefix>{prefix_alternatives})"
            ...
            if f".{expert_segment}." in key and key.endswith(".weight"):
                ...
                if which in ["gate_proj", "up_proj"]:
                    if is_gated:
                        ...
                        native_key = f"{prefix}layers.{layer_num}.{expert_segment}.up_projs"
                else:  # down_proj
