...
def _format_legacy_pickle_restore_error(path: Any, description: str) -> str:
    return (
        ...
        "weights_only=False can execute code. If this is a legacy checkpoint generated by your "
        "own trusted training run, rerun with checkpoint.allow_legacy_pickle_restore=True or "
        "migrate it in a restricted environment."
    )
...
def safe_torch_load(
    ...
    *,
    ...
    """Load a torch artifact safely by default, with explicit legacy pickle opt-in."""
    ...
    try:
        ...
        if not allow_legacy_pickle_restore:
            raise RuntimeError(_format_legacy_pickle_restore_error(path, description)) from err

    logger.warning(
        "Loading %s from %r with torch.load(weights_only=False). This can execute code; "
        "only proceed for trusted legacy checkpoints.",
        description,
        path,
    )
    unsafe_kwargs = {"weights_only": False}
    if map_location is not None:
        unsafe_kwargs["map_location"] = map_location
    return torch.load(path, **unsafe_kwargs)


def is_legacy_pickle_restore_allowed(owner: Any) -> bool:
    """Return the checkpoint legacy-pickle opt-in flag, defaulting to safe restore."""
    checkpointer = getattr(owner, "checkpointer", None)
    config = getattr(checkpointer, "config", None)
    return bool(getattr(config, "allow_legacy_pickle_restore", False))
    ...
    def load_model(
        ...
        if (
            ...
            state_dict_from_disk = _load_hf_checkpoint_preserving_dtype(
                model_path,
                allow_legacy_pickle_restore=self.config.allow_legacy_pickle_restore,
            )
...
def _load_hf_checkpoint_preserving_dtype(
    model_path: str, allow_legacy_pickle_restore: bool = False
) -> Optional[dict[str, torch.Tensor]]:
    """
    ...
    Args:
        ...
        allow_legacy_pickle_restore: Permit pickle-based torch.load fallback for legacy ``.bin`` files.
    """

    if _is_bin_checkpoint(model_path):
        return _load_hf_bin_checkpoint(model_path, allow_legacy_pickle_restore=allow_legacy_pickle_restore)
...
def _load_hf_bin_checkpoint(
    ...
    Args:
        model_path: Path to checkpoint file or directory.
        allow_legacy_pickle_restore: Permit pickle-based torch.load fallback for legacy files.
    """
    ...
    if os.path.isfile(model_path):
        return safe_torch_load(
            model_path,
            map_location="cpu",
            allow_legacy_pickle_restore=allow_legacy_pickle_restore,
