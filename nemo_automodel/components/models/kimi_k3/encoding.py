...
class _ImagePromptState:
    ...
    def assert_consumed(self) -> None:
        if self.image_prompts is None:
            return
        if self.index != len(self.image_prompts):
            raise ValueError(f"image prompt count {len(self.image_prompts)} != consumed placeholder count {self.index}")
...
def _attr(key: str, value: Any) -> list[EncodeSegment]:
    return _text(f" {key}") + _text('="') + _text(_escape_attr_value(value)) + _text('"')
...
def normalize_tool_arguments(arguments: Any) -> tuple[dict[str, Any], Optional[str]]:
    ...
    if isinstance(arguments, str):
        ...
        return parsed, None
    raise TypeError("Kimi K3 tool call arguments must be a dict or a JSON object string.")
...
def is_batched_conversation(conversation: Any) -> bool:
    return isinstance(conversation, list) and bool(conversation) and isinstance(conversation[0], list)
