...
def _iter_decoder_layers(layers: nn.Module):
    """Yield decoder layers regardless of the container type.

    Pipeline-parallel splitting converts the ``nn.ModuleList`` of decoder layers
    into an ``nn.ModuleDict`` (keyed by original layer index, holding only the
    layers assigned to this stage). Iterating an ``nn.ModuleList`` yields the
    layers, but iterating an ``nn.ModuleDict`` yields its string keys -- so the
    dict case must iterate ``.values()``.
    """
    return layers.values() if isinstance(layers, nn.ModuleDict) else layers
...
class Qwen3Model(nn.Module):
    ...
    def forward(
        ...
        **attn_kwargs: Any,
        ...
        for layer in _iter_decoder_layers(self.layers):
            h = layer(
                x=h,
                freqs_cis=freqs_cis,
                attention_mask=attention_mask,
...
class Qwen3ForCausalLM(HFCheckpointingMixin, nn.Module):
    """Qwen3 dense causal LM with THD packing, CP, TP, and PP support.
    ...
    # Pipeline parallelism: keep this model's own forward (which already nulls
    # embed_tokens/norm/lm_head per stage) instead of the generic HF pipeline
    # forward, which is incompatible with the custom THD/CP layer-calling
    # convention (freqs_cis + ``layer(x=, freqs_cis=, **attn_kwargs)``).
    # ``_pp_plan`` is the dense-model capability marker read by the capability
    # detector; the FQN-based splitter performs the actual stage partitioning.
