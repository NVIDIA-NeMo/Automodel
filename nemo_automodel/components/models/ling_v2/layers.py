...
class BailingMoeV2Attention(nn.Module):
    ...
    def __init__(self, config, backend: BackendConfig):
        ...
        self.head_dim = (config.head_dim if "head_dim" in dir(config) else None) or (
            config.hidden_size // self.num_heads
        )
        self.use_qk_norm = bool((config.use_qk_norm if "use_qk_norm" in dir(config) else False))

        attention_bias = bool((config.use_qkv_bias if "use_qkv_bias" in dir(config) else False))
        out_bias = bool((config.use_bias if "use_bias" in dir(config) else False))
