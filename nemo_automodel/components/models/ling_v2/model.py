...
class BailingMoeV2Model(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        rope_scaling = (config.rope_scaling if "rope_scaling" in dir(config) else None) or {}
        ...
        self.rotary_emb = RotaryEmbedding(
            ...
            partial_rotary_factor=float(
                (config.partial_rotary_factor if "partial_rotary_factor" in dir(config) else 1.0)
            ),
...
class BailingMoeV2ForCausalLM(HFCheckpointingMixin, nn.Module, MoEFSDPSyncMixin):
    ...
    def get_capabilities(cls, config) -> ModelCapabilities:
        ...
        """
        if (config.num_hidden_layers if "num_hidden_layers" in dir(config) else 0) > 64:
