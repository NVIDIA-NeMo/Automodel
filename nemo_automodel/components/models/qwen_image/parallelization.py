...
"""Parallelization contract for the diffusers ``QwenImageTransformer2DModel`` (Qwen-Image and Qwen-Image-Edit).

Its ``transformer_blocks`` (declared in diffusers' ``_no_split_modules``) form the ``backbone`` layer group the
parallelizer derives, and diffusers' own ``_tp_plan`` is the tensor-parallel plan; the model only asks for
whole-block activation checkpointing so the attention, image-MLP and text-MLP branches of each dual-stream
block are recomputed together.
"""
