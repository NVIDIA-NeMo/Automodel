...
class LlamaAttention(CombinedQKVAttentionMixin, nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        # Setup combined QKV projection using mixin (ALWAYS combined)
...
def build_llama_model(pretrained_model_name_or_path: str, **kwargs: Any) -> nn.Module:
    """Build a custom Llama model with combined projections.

    This function loads the config from a HuggingFace model card and builds
    a custom Llama model. ALWAYS uses combined QKV and gate_up projections for efficiency.
    ...
    Example:
        # Load with custom combined projections (ALWAYS enabled)
