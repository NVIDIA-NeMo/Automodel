...
from ._tilelang import load_tilelang
...
def _get_preprocess():
    ...
    def _preprocess(
        ...
        dtype = T.bfloat16 if dtype is None else dtype
        accum_dtype = T.float32 if accum_dtype is None else accum_dtype
...
def preprocess(
    ...
    return _get_preprocess()(B, S, H, D, block_ND, num_stages, dtype, accum_dtype)
...
def postprocess(
    ...
    return _get_postprocess()(B, S_kv, D, block_N, threads, dtype, accum_dtype)
...
def _get_bwd():
    ...
    def _bwd(
        ...
        indices_dtype = T.int32 if indices_dtype is None else indices_dtype
        dtype = T.bfloat16 if dtype is None else dtype
        accum_dtype = T.float32 if accum_dtype is None else accum_dtype
...
def bwd(
    ...
    return _get_bwd()(
        B, S, S_kv, H, D, topk, sm_scale, block_size, num_stages, threads, indices_dtype, dtype, accum_dtype
    )
