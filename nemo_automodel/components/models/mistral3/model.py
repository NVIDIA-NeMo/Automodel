"""
Custom Mistral3 integration for NeMo Automodel.

This module:
- Imports the VLM wrapper `Mistral3ForConditionalGeneration` from transformers_bak.
- Vendors the text-only Ministral3 config/model so the registry can resolve the
  decoder without relying on transformers_5.
- Patches AutoModel.from_config to return the vendored Ministral3Model when
  config.model_type == "ministral3".
- Exposes ModelClass for the NeMo ModelRegistry.
"""
