...
class MiniMaxM3MLP(nn.Module):
    ...
    def __init__(self, config: Any, intermediate_size: int, backend: BackendConfig):
        super().__init__()
        self.alpha = float((config.swiglu_alpha if hasattr(config, "swiglu_alpha") else 1.702))
        self.limit = float((config.swiglu_limit if hasattr(config, "swiglu_limit") else 7.0))
    ...
    *blocks* to attend to (block-level DeepSeek-style sparse attention). M3 sets
    ...
    def __init__(
        ...
        *,
        ...
        assert not (config.attention_output_gate if hasattr(config, "attention_output_gate") else False), (
            "MiniMax M3 attention_output_gate is not implemented"
        )
        qk_norm_type = config.qk_norm_type if hasattr(config, "qk_norm_type") else "per_head"
...
class Block(nn.Module):
    ...
    def __init__(self, layer_idx: int, config: Any, moe_config: MoEConfig, backend: BackendConfig):
        ...
        sparse_cfg = config.sparse_attention_config if hasattr(config, "sparse_attention_config") else None
        ...
        moe_layer_freq = config.moe_layer_freq if hasattr(config, "moe_layer_freq") else None
        self.is_moe_layer = True if moe_layer_freq is None else moe_layer_freq[layer_idx] != 0

        if self.is_moe_layer:
            self.mlp = MoE(moe_config, backend)
            n_shared = (config.n_shared_experts if hasattr(config, "n_shared_experts") else 0) or 0
            if n_shared > 0:
                shared_inter = (
                    config.shared_intermediate_size
                    if hasattr(config, "shared_intermediate_size")
