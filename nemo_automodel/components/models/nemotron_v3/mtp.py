...
def _resolve_block_types_per_sublayer(config) -> list[str] | None:
    ...
    """
    pattern = config.mtp_hybrid_override_pattern if "mtp_hybrid_override_pattern" in dir(config) else None
    if pattern:
        return parse_mtp_layer_pattern(pattern)
    block_types = config.mtp_layers_block_type if "mtp_layers_block_type" in dir(config) else None
    if block_types:
...
def build_mtp_config_from_hf(
    ...
    *,
    ...
    pattern = (config.mtp_hybrid_override_pattern if "mtp_hybrid_override_pattern" in dir(config) else None) or ""
    if not pattern:
        block_types = config.mtp_layers_block_type if "mtp_layers_block_type" in dir(config) else None
        if block_types:
