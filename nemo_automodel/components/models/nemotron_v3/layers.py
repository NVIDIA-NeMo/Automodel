...
class NemotronV3Mamba2Mixer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if seq_idx is None and hidden_states.dim() == 3 and batch_size > 1:
            # BSHD with B > 1 (e.g. default-collater validation): rows are
            # independent sequences, so each row is a single sub-sequence.
            # cu_seqlens is a single-flattened-stream descriptor and is
            # meaningless across rows (model.py no longer derives it from a 2D
            # mask at B>1); this also ignores any cu_seqlens passed in here.
            # Intra-row neat packing is handled separately via _packed_seq_ids.
            seq_idx = torch.zeros(batch_size, seq_len, device=hidden_states.device, dtype=torch.int32)
        elif seq_idx is None and "cu_seqlens" in kwargs:
