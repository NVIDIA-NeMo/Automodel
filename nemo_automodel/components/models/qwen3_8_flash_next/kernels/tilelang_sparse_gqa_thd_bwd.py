...
#
# Direct-THD implementation derived from the Qwen3.8-Flash-Next sparse-GQA kernel at Automodel commit
...
"""Direct packed-THD TileLang backward kernel for Qwen3.8-Flash-Next sparse GQA."""
...
def sparse_gqa_thd_bwd_cast(
    ...
    assert dtype == T.bfloat16
    assert accum_dtype == T.float32
    tensor_shape = [kv_tokens, kv_heads, dim]
    ...
    def cast_kernel(
        ...
        with T.Kernel(T.ceildiv(kv_tokens, block_n), kv_heads, threads=threads) as (
            token_block,
            kv_head_idx,
        ):
            T.copy(
...
def sparse_gqa_thd_bwd(
    ...
    kv_shape = [kv_tokens, kv_heads, dim]
...
def sparse_gqa_thd_bwd_interface(
    ...
    *,
    ...
    Args:
        query: BF16 CUDA queries shaped ``[Tq, 24, 256]``.
        key: BF16 CUDA keys shaped ``[Tkv, 2, 256]``. ``Tkv`` may differ from
            ``Tq``; under context parallelism it is the gathered global count.
        value: BF16 CUDA values shaped ``[Tkv, 2, 256]``.
        output: BF16 forward output shaped ``[Tq, 24, 256]``.
        doutput: BF16 upstream gradient shaped ``[Tq, 24, 256]``.
        token_ids: INT32/INT64 CUDA route IDs shaped ``[Tq, K]`` or
            ``[Tq, 1, K]``, with valid IDs in ``[0, Tkv)``.
        lse: FP32 base-2 forward LSE shaped ``[Tq, 2, 16]``.
    ...
    if tokens < 1:
        raise ValueError("direct Qwen3.8-Flash-Next THD requires at least one token")
    if key.ndim != 3 or key.shape[0] < 1 or key.shape[1:] != (_KV_HEADS, _HEAD_DIM) or value.shape != key.shape:
        raise ValueError(
            f"key/value must have matching [Tkv, {_KV_HEADS}, {_HEAD_DIM}] shapes; "
    ...
    if any(tensor.dtype != torch.bfloat16 for tensor in compute_tensors):
        raise TypeError("direct Qwen3.8-Flash-Next THD backward requires BF16 compute tensors")
    ...
    if not all(tensor.is_cuda for tensor in (*compute_tensors, token_ids, lse)):
        raise RuntimeError("direct Qwen3.8-Flash-Next THD backward requires CUDA tensors")
    if any(tensor.device != query.device for tensor in (*compute_tensors[1:], token_ids, lse)):
        raise ValueError("all direct Qwen3.8-Flash-Next THD backward tensors must share one CUDA device")
    ...
    if topk > _MAX_TOPK:
        raise ValueError(f"Qwen3.8-Flash-Next THD sparse GQA supports at most {_MAX_TOPK} selected slots, got {topk}")
    padded_topk = math.ceil(topk / block_size) * block_size
    if padded_topk != topk:
        token_ids = torch.nn.functional.pad(token_ids, (0, padded_topk - topk), value=-1)
    valid_mask = ((token_ids >= 0) & (token_ids < kv_tokens)).to(torch.int32).contiguous()
    safe_ids = token_ids.clamp(min=0, max=kv_tokens - 1).to(torch.int32).contiguous()
    ...
    cast_kernel = sparse_gqa_thd_bwd_cast(kv_tokens)
