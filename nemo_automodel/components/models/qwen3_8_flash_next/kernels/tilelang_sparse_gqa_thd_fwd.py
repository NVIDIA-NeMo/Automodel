...
"""Direct packed-THD TileLang forward kernel for Qwen3.8-Flash-Next sparse GQA.
...
def _normalize_token_ids(token_ids: torch.Tensor, tokens: int) -> torch.Tensor:
    ...
    if token_ids.shape[1] > _MAX_TOPK:
        raise ValueError(
            f"Qwen3.8-Flash-Next THD sparse GQA supports at most {_MAX_TOPK} selected slots, got {token_ids.shape[1]}"
        )
    return token_ids
    ...
    *,
    ...
    Args:
        ...
        token_ids: INT32/INT64 CUDA route IDs shaped ``[Tq, K]`` or
            ``[Tq, 1, K]``, where ``1 <= K <= 2051``. Valid IDs index into
            ``[0, Tkv)``.
    ...
    if key.ndim != 3 or key.shape[0] < 1 or key.shape[1:] != (_KV_HEADS, dim):
        raise ValueError(f"key/value must be [Tkv, {_KV_HEADS}, D] with the same D as query; got {tuple(key.shape)}")
    kv_tokens = key.shape[0]
    if (query_heads, dim) != (_QUERY_HEADS, _HEAD_DIM):
        raise ValueError(
            f"direct Qwen3.8-Flash-Next THD requires Hq={_QUERY_HEADS}, Hkv={_KV_HEADS}, D={_HEAD_DIM}; "
    ...
    if any(tensor.dtype != torch.bfloat16 for tensor in (query, key, value)):
        raise TypeError("direct Qwen3.8-Flash-Next THD sparse GQA requires BF16 query, key, and value")
    if not all(tensor.is_cuda for tensor in (query, key, value, token_ids)):
        raise RuntimeError("direct Qwen3.8-Flash-Next THD sparse GQA requires CUDA tensors")
