...
"""Autograd adapter for native Qwen3.8-Flash-Next direct-THD sparse GQA kernels."""
...
class _Qwen3_8_FlashNextSparseGQAThdAttention(torch.autograd.Function):
...
def tilelang_sparse_gqa_thd_attention(
    ...
    """Run direct packed-THD Qwen3.8-Flash-Next sparse GQA.

    Args:
        query: BF16 CUDA queries shaped ``[Tq, 24, 256]`` with ``Tq >= 1``.
        key: BF16 CUDA keys shaped ``[Tkv, 2, 256]``. ``Tkv`` may differ from
            ``Tq``; under context parallelism the queries are the rank-local
            packed shard while K/V and route IDs use the gathered global
            token space.
        value: BF16 CUDA values shaped ``[Tkv, 2, 256]``.
        token_ids: Global flattened route IDs shaped ``[Tq, K]`` or
            ``[Tq, 1, K]``, where ``1 <= K <= 2051``. Negative and
    ...
    Returns:
        BF16 output shaped ``[Tq, 24, 256]``. The custom backward returns Q/K/V
    ...
    return _Qwen3_8_FlashNextSparseGQAThdAttention.apply(query, key, value, normalized_ids, scale)
