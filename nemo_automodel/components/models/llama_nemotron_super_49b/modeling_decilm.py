    ...
    def __init__(self, config: DeciLMConfig, layer_idx: int):
        ...
        if not self.attention_config.no_op:
            ...
            if not self.attention_config.replace_with_linear:
                self.self_attn = DeciLMAttention(config=config, attention_config=self.attention_config, layer_idx=layer_idx)
            else:
...
class DeciLMModel(DeciLMPreTrainedModel):
    ...
    def forward(
        ...
        for layer in self.layers:
            ...
            if attn and isinstance(attn, DeciLMAttention):
                llama_style_attention_config = attn.config
                causal_mask = create_causal_mask(
                        config=llama_style_attention_config,
                ...
                break
        else:  # no attention layers in model
            causal_mask = None
