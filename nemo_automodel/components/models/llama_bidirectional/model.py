...
class LlamaBidirectionalForSequenceClassification(LlamaPreTrainedModel):
    ...
    def _init_weights(self, module):
        if module is self.score:
            ...
            std = (self.config.initializer_range if "initializer_range" in dir(self.config) else 0.02) or 0.02
    ...
    def forward(
        ...
        **kwargs,
        ...
        pooled_hidden_states = _pool(
            ...
            attention_mask=attention_mask,
            pool_type=(self.config.pooling if "pooling" in dir(self.config) else "avg"),
        )

        pooled_logits = self.score(pooled_hidden_states)
        pooled_logits = pooled_logits / (self.config.temperature if "temperature" in dir(self.config) else 1.0)
