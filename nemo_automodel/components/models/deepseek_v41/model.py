...
"""Trainable DeepSeek V4.1 backbone with explicit cross-layer attention state."""
...
class DeepseekV41Model(nn.Module):
    """Text embeddings and the 40-layer CED stack used for full-sequence training."""
...
class DeepseekV41ForCausalLM(HFCheckpointingMixin, PreTrainedModel, MoEFSDPSyncMixin):
    """Native backbone fine-tuning for the released DeepSeek-V4.1-Flash checkpoint."""
    ...
    class ModelCapabilities:
        """Parallel axes enabled after their corresponding validation gates."""
    ...
    def _nemo_prepare_model_owned_dtensors(self, fsdp_mesh: DeviceMesh) -> set[nn.Parameter]:
        """Attach owner-local Engram weights to their global row-sharded DTensors.

        Args:
            fsdp_mesh: One-dimensional owner mesh; ranks must have exactly the
                Engram process group's ordering.

        Returns:
            Registered global parameters [padded_rows, embedding_dim] with Shard(0),
            consumed locally by Engram and therefore excluded from FSDP gathers.
        """
        owned = set()
        for layer in self.model.layers.values():
            if layer.engram is not None:
                owned.add(layer.engram.embed.parallelize_weight(fsdp_mesh))
        return owned
