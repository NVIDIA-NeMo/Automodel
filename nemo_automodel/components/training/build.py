...
def build_vlm_model(
    *,
    ...
    ``model_factory`` must be a ``NeMoAutoModelFor{ImageTextToText,MultimodalLM,
    CausalLM}.from_*`` classmethod — those classes apply distributed / PEFT /
    FP8 / compile / MoE / freeze logic internally. No fallback path.

    Args:
        model_factory: Resolved callable producing the model.
        model_kwargs: Keyword arguments forwarded to ``model_factory``.
        seed: Random seed.
        peft_config: Instantiated PEFT config or None.
        freeze_config: Plain dict describing freeze rules, or None.
        fp8_config: Pre-built FP8Config or None.
        compile_config: Pre-built CompileConfig or None.
        device_mesh: Device mesh for distributed training.
        moe_mesh: MoE mesh.
    ...
    with ScopedRNG(seed=seed, ranked=True):
        ...
        return model_factory(**model_kwargs, **infra_kwargs)
