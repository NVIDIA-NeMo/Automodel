...
class JetSpecTrainerModule(DFlashTrainerModule):
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            # Acceptance proxy: fraction of supervised positions where the draft's
            # greedy token matches the TARGET's greedy token. This is the greedy
            # speculative-decoding accept condition (paper Eq. 12). Because JetSpec
            # distills toward the target's distribution, the target's argmax is the
            # right reference; comparing against the raw ground-truth token (DFlash's
            # hard-label metric) understates agreement, especially when the training
            # data was regenerated by a different model than the target.
            target_argmax = teacher_logits.argmax(dim=-1)
            correct = (student_logits.argmax(dim=-1) == target_argmax) & valid
            accuracy = correct.sum().float() / valid_tokens.clamp_min(1).float()
