...
Each shard has the following layout::
    ...
    int32[256] header
        ...
        header[1] == 1         (version)
        header[2] == num_tokens (uint16 tokens that follow)

    uint16[num_tokens] tokens

The dataset streams one contiguous *seq_len* token slice at a time and
returns the pair ``(inputs, targets)`` where ``targets`` is shifted by one
...
def _peek_num_tokens(path: str | os.PathLike) -> int:
    """Return *num_tokens* from the shard header without mapping tokens."""
...
def load_bin_shard(path: str | os.PathLike) -> torch.Tensor:
    """Memory-map a *.bin* shard and return it as a 1-D ``torch.uint16`` tensor.
    ...
    # Memory-map the tokens (uint16).  Offset skips the 256×4-byte header.
    tokens_np = np.memmap(
        path, dtype=np.uint16, mode="r", offset=HEADER_BYTES, shape=(num_tokens,)
    )
    # uint16 CPU tensor shares memory with np.memmap → zero-copy
