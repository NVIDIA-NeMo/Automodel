...
def make_fleurs_dataset(
    path_or_dataset="google/fleurs",
    language="en_us",
    split="train",
    **kwargs,
):
    """Load FLEURS dataset for audio-to-text fine-tuning.

    FLEURS has parquet files in the repo but also a loading script which newer
    datasets versions reject — we download the parquet files directly.

    Args:
        path_or_dataset: HF dataset ID (default "google/fleurs").
        language: FLEURS config (e.g. "en_us", "fr_fr", "de_de").
        split: Split name ("train", "validation", "test").

    Returns:
        List of formatted samples with "conversation" and "audio" keys.
    """
    import glob
    from huggingface_hub import snapshot_download

    parquet_files = []
    # First, check existing local snapshots (the latest snapshot may only have README.md
    # and the loading script, while an older snapshot has the actual parquet files).
    hf_home = os.environ.get("HF_HOME") or os.path.expanduser("~/.cache/huggingface")
    repo_cache = os.path.join(
        hf_home, "hub", f"datasets--{path_or_dataset.replace('/', '--')}", "snapshots"
    )
    if os.path.isdir(repo_cache):
        candidates = sorted(
            glob.glob(
                os.path.join(repo_cache, "*", language, f"fleurs-{split}*.parquet")
            )
        )
        if candidates:
            parquet_files = candidates

    if not parquet_files:
        snap_dir = snapshot_download(
            repo_id=path_or_dataset,
            repo_type="dataset",
            allow_patterns=[f"{language}/fleurs-{split}*.parquet"],
        )
        parquet_files = sorted(
            glob.glob(os.path.join(snap_dir, language, f"fleurs-{split}*.parquet"))
        )
    if not parquet_files:
        raise FileNotFoundError(
            f"No parquet files found for {path_or_dataset} {language}/{split}"
        )
    ...
    def format(example):
        return {
            "conversation": [
                {"role": "user", "content": "<so_embedding>\nTranscribe the audio clip."},
                {"role": "assistant", "content": example["transcription"]},
            ],
            "audio": (example["audio"]["array"], example["audio"]["sampling_rate"]),
        }
...
def make_cv17_en_accent_dataset(
    path_or_dataset="fixie-ai/common_voice_17_0",
    language="en",
    accent="Scottish English",
    split="train",
    **kwargs,
):
    """Load CommonVoice 17 English filtered by accent, format for NemotronOmni audio ASR.

    Args:
        path_or_dataset: HF dataset ID (requires gated access).
        language: Language config (e.g. "en").
        accent: Accent substring to filter samples by (matched against the "accents" field).
        split: Split name ("train", "validation", "test").

    Returns:
        List of formatted samples with "conversation" and "audio" keys.
    """
    # Load only the requested split's parquet files directly from the HF cache to
    # avoid triggering downloads of other splits (e.g. "validated" is ~138 GB and
    # is NOT needed for our Scottish subset). Fall back to load_dataset otherwise.
    dataset = None
    try:
        import glob
        from huggingface_hub import snapshot_download
        ...
        snap_dir = snapshot_download(
            repo_id=path_or_dataset,
            repo_type="dataset",
            allow_patterns=[f"{language}/{split}-*.parquet", "README.md"],
        )
        parquet_files = sorted(glob.glob(os.path.join(snap_dir, language, f"{split}-*.parquet")))
        if parquet_files:
        ...
        logger = logging.getLogger(__name__)
        logger.warning(f"Direct parquet load failed ({e}); falling back to load_dataset")

    if dataset is None:
        dataset = load_dataset(path_or_dataset, language, split=split)

    def _has_accent(ex):
        accents = ex.get("accents") or ex.get("accent") or ""
        return accent.lower() in (accents or "").lower()

    dataset = dataset.filter(_has_accent)

    def format(example):
        return {
            "conversation": [
                {"role": "user", "content": "<so_embedding>\nTranscribe the English audio clip."},
