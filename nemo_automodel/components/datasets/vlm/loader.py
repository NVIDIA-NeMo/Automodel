...
import logging
from collections.abc import Callable, MutableMapping, Sequence, Sized
...
class VlmCollatorConfig:
    ...
    def build(self, *, tokenizer: ProcessorMixin | None) -> VlmCollateFn:
        """Bind the runtime multimodal tokenizer to the configured collator."""
        return partial(self.factory, processor=tokenizer, **self.kwargs)
...
class VlmDataloaderConfig:
    ...
    def _build_source(
        ...
        *,
        ...
        """Build the raw dataset under the caller-owned ordering context."""
        with dataset_build_context or nullcontext():
            if self.chat_template is not None and tokenizer is not None:
                ...
                text_tokenizer.chat_template = chat_template
            ...
            if isinstance(self.dataset_config, RankedVlmDatasetConfig):
                ...
                dataset = self.dataset_config.build(tokenizer=tokenizer)
            else:
    ...
    def build(
        ...
        *,
        ...
        """Build the dataset wrappers, sampler, collator, and dataloader.

        Args:
            tokenizer: Runtime multimodal tokenizer or processor.
        ...
        Returns:
            Stateful multimodal dataloader.
        """
        dataset = self._build_source(
            tokenizer=tokenizer,
            dp_rank=dp_rank,
            dp_world_size=dp_world_size,
            dataset_build_context=dataset_build_context,
        )
        raw_dataset = cast(Dataset | Sequence[dict[str, object]], dataset)

        if self.pretokenization is not None:
            if tokenizer is None:
                raise ValueError("VLM pretokenization requires a processor")
            dataset = self.pretokenization.build(dataset=raw_dataset, processor=tokenizer)

        if self.packing is not None:
            if self.pretokenization is None:
                raise ValueError("VLM neat packing requires pretokenization")
            text_tokenizer = getattr(tokenizer, "tokenizer", None)
            padding_idx = getattr(text_tokenizer, "pad_token_id", 0) or 0
            dataset = self.packing.build(
                dataset=cast(PreTokenizedDatasetWrapper, dataset),
            ...
            collate_fn = self.collator.build(tokenizer=tokenizer)
        elif self.pretokenization is not None:
            collate_fn = partial(pad_collate_fn, processor=tokenizer)
        else:
            processor_type = type(tokenizer).__name__
            if processor_type not in COLLATE_FNS:
                logger.warning("Using %s with the default VLM collator", processor_type)
                processor_type = "default"
            collate_fn = partial(COLLATE_FNS[processor_type], processor=tokenizer)

        robust_collate = getattr(dataset, "robust_collate", None)
        if callable(robust_collate):
            collate_fn = robust_collate(collate_fn)
        if pp_n_microbatches is not None:
            collate_fn = wrap_vlm_collate_for_pp(
                cast(Callable[[Any], MutableMapping[str, Any]], collate_fn),
        ...
        torch_dataset = cast(Dataset, dataset)
