...
def build_model_and_optimizer(
    ...
    loss_fn=None,
    parallelize_fn=None,
) -> tuple[nn.Module, "Optimizer", nn.Module]:  # noqa: F821
    ...
    # Apply fully_shard_optimizer for MegatronFSDPManager
    if isinstance(model_wrapper, MegatronFSDPManager):
        optimizer = fully_shard_optimizer(model, optimizer)
...
class FinetuneRecipeForVLM(BaseRecipe):
    ...
    def setup(self):
        ...
        self.model, self.optimizer, self.loss_fn = build_model_and_optimizer(
            self.cfg.model,
            self.cfg.optimizer,
