...
class BenchmarkingRecipeForNextTokenPrediction(TrainFinetuneRecipeForNextTokenPrediction):
    ...
    def run_benchmark(self):
        ...
        if completed_profile is not None:
            ...
            events = sorted(prof.key_averages(), key=lambda event: event.self_device_time_total, reverse=True)
            summary = [
                {
                    "name": event.key,
                    "count": event.count,
            ...
            summary_path = output_dir / f"rank{rank}_iteration{profiled_iteration}_summary.json"
            summary_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")
            logger.info("Rank %d | torch.profiler trace: %s", rank, trace_path)
            logger.info("Rank %d | torch.profiler summary: %s", rank, json.dumps(summary, separators=(",", ":")))
