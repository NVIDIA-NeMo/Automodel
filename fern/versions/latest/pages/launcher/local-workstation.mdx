...
position: 2
---
Use this guide for local, single-node workflows on a workstation or an interactive Slurm allocation. For setup details, refer to our [Installation Guide](/latest/guides/installation).
For batch multi-node jobs, see the [Slurm](/latest/launcher/slurm) or [SkyPilot](/latest/launcher/skypilot) guides.

NeMo AutoModel uses recipes to run end-to-end workflows. If you're new to recipes, see the [Repository Structure](/latest/repository-structure) guide.
...
The AutoModel CLI is the preferred method for most users. It offers a unified interface to launch training scaling from a local workstation (this guide) to large clusters (see our [cluster guide](/latest/launcher/slurm)).
...
Looking for Slurm or cloud training? See [Slurm](/latest/launcher/slurm) or [SkyPilot](/latest/launcher/skypilot).
...
This approach requires that you have already installed NeMo AutoModel and its dependencies in your Python environment (see the [installation guide](/latest/guides/installation) for details).
...
All approaches use the same configuration files and provide the same training capabilities on a single node. For multi-node training, see [Run on a Cluster](/latest/launcher/slurm).
