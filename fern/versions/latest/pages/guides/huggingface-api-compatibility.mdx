...
<Note>
If you are running Transformers v5 in another environment, you can still use NeMo AutoModel-produced consolidated checkpoints with Transformers' standard loading APIs. For details on the checkpoint layouts, see [checkpointing](/latest/guides/checkpointing).
...
- See [checkpointing](/latest/guides/checkpointing) for checkpoint formats and example directory layouts.
- See [model coverage](/latest/model-coverage/overview) for notes on how model support depends on the pinned Transformers version.
