...
The specific example here fine-tunes a [Llama-3.2-1B](https://huggingface.co/meta-llama/Llama-3.2-1B) model using the [SQuAD dataset](https://huggingface.co/datasets/rajpurkar/squad) from Hugging Face, but any AutoModel functionality (for example, [model pre-training](/latest/guides/llm/pretraining), [VLMs](/latest/model-coverage/vlm), [other supported models](/latest/model-coverage/overview)) can also be run on Databricks.
...
- `--checkpoint.checkpoint_dir`: Tells AutoModel where to [save model checkpoints](/latest/guides/checkpointing) from training. We recommend saving model checkpoints in a Databricks Unity Catalog [volume](https://docs.databricks.com/aws/en/volumes/).
...
![Single GPU utilization of ~95% during model training.](/latest/databricks-gpu-metrics-single.png)
...
![Multi-GPU, single-node utilization of ~95% during model training.](/latest/databricks-gpu-metrics-multi.png)
...
For more details on MLflow configuration options and best practices, see the [MLflow logging guide](/latest/guides/mlflow-logging).
...
While the example here fine-tunes a Llama-3.2-1B model using the SQuAD dataset, any supported AutoModel functionality (like model pre-training, VLMs, etc.) can also run, and scale, on Databricks. Check out [additional recipes and end-to-end examples](/latest/guides/overview) to learn more.
