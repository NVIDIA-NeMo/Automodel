...
In this guide, we will use an interactive environment to install NeMo AutoModel from Git. You can also install NeMo AutoModel from PyPI or use our bi-monthly Docker container (see the [Installation Guide](/latest/guides/installation)).
...
<Tip>
To run this guide on a single GPU, use the single-GPU command in the **Launch Training** section below and scale down the YAML (for example, reduce `step_scheduler.global_batch_size` / `local_batch_size`, and shrink the model using `model.n_layer` / `model.n_embd` / `model.n_head`). For more launch patterns, see [Run on Your Local Workstation](/latest/launcher/local-workstation).
...
For larger datasets, please see [pretraining.md](/latest/guides/llm/pretraining) which supports sharded preprocessing without these constraints.
...
<Tip>
To train on more than 2B tokens from FineWeb, see [pretraining.md](/latest/guides/llm/pretraining) which uses Megatron Core's sharded dataset format without file size constraints.
