...
When training inside a Docker container (see [Installation Guide](/latest/guides/installation)), any files written to the container's filesystem are lost when the container exits (especially with `--rm`). To keep your checkpoints, you must **bind-mount a host directory** to the checkpoint path before starting the container:
...
<Tip>
Mount additional host directories for datasets and the Hugging Face model cache to avoid re-downloading large models across container restarts. See the [Installation Guide](/latest/guides/installation) for a complete `docker run` example with all recommended mounts.
