...
|------------|------|--------|---------|
| `AutoModelForCausalLM` | Text Generation (LLM) | Supported | See [LLM model list](/latest/model-coverage/llm). |
| `AutoModelForImageTextToText` | Image-Text-to-Text (VLM) | Supported | See [VLM model list](/latest/model-coverage/vlm). |
| `AutoModelForSequenceClassification` | Sequence Classification | WIP | Early support; interfaces may change. |
| Diffusers Pipelines | Diffusion Generation (T2I, T2V) | Supported | See [Diffusion model list](/latest/model-coverage/diffusion). |

## Release Log

The table below tracks when model support and key features were added across NeMo AutoModel releases. For the full list of tested architectures and example configs, see the [LLM](/latest/model-coverage/llm) and [VLM](/latest/model-coverage/vlm) pages.
...
If a model from the Hub doesn't work as expected, see the [Troubleshooting](/latest/model-coverage/troubleshooting) guide for common issues and solutions.
