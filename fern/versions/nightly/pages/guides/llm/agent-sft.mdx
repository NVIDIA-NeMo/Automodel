...
- **Full-parameter SFT**: Qwen2.5-3B fits comfortably on a single 80 GB GPU (8 GPUs shown below for speed) with FSDP2.
...
sample = dataset[0]
print("keys:", list(sample.keys()))                       # input_ids, labels, attention_mask
...
The base model typically answers in prose or emits a wrongly-named / malformed call instead of a clean `get_stock_price(symbol="AAPL")` tool call.
