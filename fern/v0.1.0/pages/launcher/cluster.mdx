---
title: "Run on a Cluster (Slurm / Multi-node)"
description: ""
---

In this guide, you will learn how to submit distributed training jobs on Slurm clusters (single- or multi-node). For single-node workstation usage, see [Run on Your Local Workstation](/./local-workstation). For setup details, refer to our [Installation Guide](/../guides/installation).

NeMo Automodel uses recipes to run end-to-end workflows. If you're new to recipes, see the [Repository Structure](/../repository-structure) guide.
...
The Automodel CLI is the preferred method for most users. It provides a unified interface for running jobs, from local environments (e.g., [workstation](/./local-workstation)) to large clusters (e.g., Slurm batch jobs). The CLI will automatically submit the job to Slurm and handle the distributed setup. The above example launches one node with eight workers per node using torchrun (`--nproc_per_node=8`). The Slurm script itself uses `#SBATCH --ntasks-per-node 1`, and when `gpus_per_node` is set, it adds `#SBATCH --gpus-per-node=8` as well.
...
When developing on clusters, you can use `uv` to prepare and test scripts locally. For single-node `torchrun` examples, see [Run on Your Local Workstation](/./local-workstation). Cluster execution should be done through the CLI with `slurm` configs above.
...
For cluster usage, prefer submitting via the CLI with `slurm` configuration. Direct `torchrun` is recommended for single-node development; see [Run on Your Local Workstation](/./local-workstation).
...
All approaches use the same configuration files. For single-node workflows, see our [Run on Your Local Workstation](/./local-workstation) guide.
