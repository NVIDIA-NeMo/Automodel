...
Use this guide for local, single-node workflows. For setup details, refer to our [Installation Guide](/../guides/installation).
For executing distributed multi-node jobs, please refer to our [Run on a Cluster](/./cluster) guide.
...
The Automodel CLI is the preferred method for most users. It offers a unified interface to launch training scaling from a local workstation (this guide) to large clusters (see our [cluster guide](/./cluster)).
...
Looking for Slurm or multi-node? See [Run on a Cluster](/./cluster).
...
This approach requires that you have already installed NeMo Automodel and its dependencies in your Python environment (see the [installation guide](/../guides/installation) for details).
...
All approaches use the same configuration files and provide the same training capabilities on a single node. For Slurm-based multi-node training, see [Run on a Cluster](/./cluster).
