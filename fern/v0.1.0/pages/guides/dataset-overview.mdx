...
- See also: [LLM datasets](/llm/dataset), [VLM datasets](/vlm/dataset), and [Biencoder retrieval dataset](/llm/retrieval-dataset) for deeper, task-specific guides.
...
dataset:
  ...
  start_of_turn_token: "<|assistant|>"
```
See the detailed guide, [Column-Mapped Text Instruction Dataset](/llm/column-mapped-text-instruction-dataset), for more information.
...
```
See the [Function Calling guide](/llm/toolcalling) for an end-to-end example with FunctionGemma.
...
collate_fn:
  ...
  p_max_len: 512
```
See the detailed guide, [Biencoder retrieval dataset](/llm/retrieval-dataset), for more information.
...
```
See the detailed [pretraining guide](/llm/pretraining), which uses MegatronPretraining data.
...
See [Gemma-3n](/omni/gemma3-3n) and [VLM dataset](/vlm/dataset) for end-to-end examples.
