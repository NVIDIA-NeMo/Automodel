...
├── versions/
│   ├── v0.4.yml              # Canonical nav for the v0.4 train (source of truth)
│   ├── latest.yml            # GA alias — copy of v0.4.yml; bump on next GA cut
│   ├── nightly.yml           # Bleeding-edge alias — currently mirrors v0.4 until nightly content lands
│   └── v0.4/pages/           # MDX content tree (130+ pages)
...
────────────────────────────────────────────────────────  ─────────────────────────────────────────────────
fern/versions/v0.4/pages/get-started/installation.mdx     docs.nvidia.com/nemo/automodel/latest/get-started/installation
                                                          docs.nvidia.com/nemo/automodel/v0.4/get-started/installation
                                                          docs.nvidia.com/nemo/automodel/nightly/get-started/installation
```

The same MDX file backs all three version slugs because `latest.yml` and `nightly.yml` are copies of `v0.4.yml` whose `path:` lines point at `./v0.4/pages/...`.
...
`v0.4.yml` is canonical; `latest.yml` and `nightly.yml` are content copies that point at `./v0.4/pages/...`. When the next GA cuts:

1. `cp -r versions/v0.4 versions/v0.5` — frozen snapshot
2. `cp versions/v0.4.yml versions/v0.5.yml`, then sed `./v0.4/` → `./v0.5/` in the new file
3. Update `versions/latest.yml` to point at `./v0.5/pages/...` (`cp` it from the new `v0.5.yml` is fine)
4. Add the new frozen-pin entry to `docs.yml` `versions:` (`display-name: "0.5.0"`, `slug: v0.5`, `availability: stable`)
5. Promote nightly to `availability: stable` if/when it goes GA
